...
const mcp = mode === "native-repl" ? { command: nodeConfig.command, args: nodeConfig.args, env: nodeConfig.env, enabled_tools: ["js", "js_reset"] }
  : mode === "opensky" ? { command: process.execPath, args: ["--import", resolve("node_modules/tsx/dist/loader.mjs"), resolve("evals/parity/opensky-mcp.ts")], env: { OPENSKY_HOME: join(artifacts, "sdk-home"), OPENSKY_DRIVER_BINARY: process.env.OPENSKY_DRIVER_BINARY! } }
...
const result = await withOwnedMacApp({ bundleId: "com.apple.calculator", appPath: "/System/Applications/Calculator.app", artifacts }, () => runCodex({
  ...
  timeoutMs: 120_000, maxToolCalls: 2,
  mcp,
