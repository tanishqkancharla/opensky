  ...
  const tools = [defineTool({
    ...
    execute(_id, params) {
      return cells.start(() => lifecycle.run("tool execution", () => {
        ...
        return execution;
      }), params.yield_time_ms).then(result => ({ ...result, details: result.details }));
    },
  }), defineTool({
    name: "cua_repl_wait",
    label: "cua_repl_wait",
    ...
    execute(_id, params) { return cells.wait(params.cell_id, params.yield_time_ms).then(result => ({ ...result, details: result.details })); },
  })];
