...
V14 completed **20 matched Linux tasks / 40 arms** with clean final measurement verification: OpenSky passed **18/20 (90%)** and native Computer Use passed **16/20 (80%)**. OpenSky passed **16/16 native-success tasks**; there are no native-only successes in this measurement. The related CI run [34782011989](https://github.com/tanishqkancharla/opensky/actions/runs/34782011989) is **not passed**: all three evaluate jobs failed at the frozen native `.deb` checksum before public SDK tests. The driver build passed with the V14-pinned binary SHA. The repaired [SDK gate](https://github.com/tanishqkancharla/opensky/actions/runs/34782566739) then passed 20 of 27 cases. Six failures came from obsolete dialog setup/assertions, corrected and verified in four focused warm tests. One intermittent Unicode typing failure remains under investigation; integration is not accepted.
...
V14 controller packaging remains incomplete. The repository’s [parity evaluation documentation](../evals/parity/README.md) describes its generic framework, but no supported public command recreates this frozen paired 20-task measurement. Do not substitute a nonpaired evaluator for V14 reproduction.
