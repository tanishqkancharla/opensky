...
These repeat results remain separate from the original 20-task measurement. Native failed the repeated background task that it passed in V14, demonstrating run variability. Saved-file inspection shows native chose other green shades (`81d41a` and, in the five-task repeat, `5eb91e`) while the reference is `00a933`. The task only says “green”; this exact-color scoring ambiguity means those failures do not establish inability to perform the requested edit. The subsequent five-task repeat completed ten valid arms: **OpenSky 5/5, native 4/5**, with 15 tasks unrun and **$2.772508** evaluation inference. Its only native failure was the green-shade case described above. The full 20-task repeat then completed **OpenSky 16/20 (80%), native 14/20 (70%)**. All 40 scored arms passed a fresh evidence audit. OpenSky passed duplicate-slides, split-fields and freeze-panes tasks that native failed; native alone passed the dropdown task. Both failed the background, subscript and slide text-size tasks. The original 18/20 versus 16/20 result remains unchanged. Two full runs favor OpenSky on this fixed benchmark; they do not establish general or statistical parity. The wrapper requires the existing provisioned stage and does not provision a new host.

The full repeat retained one unscored setup failure: its editor task was initially routed through the office launcher. The corrected controller used the existing verified editor fixture, retained that attempt, and continued without repeating the first five completed pairs. The new editor setup matched the original environment except capture time. All test containers were removed and the worker lock was released. Scored-arm evaluation inference was **$12.310552** (OpenSky $8.634816; native $3.675736). Accounting also retains a separate **$5 conservative charge** for the pre-agent setup failure; this is not evidence of model usage. Both figures exclude coding inference and infrastructure.

Saved-file diagnoses explain the repeat failures without changing their scores. Both subscript agents changed only the first occurrence while the reference expects all eight. OpenSky created the requested dropdown validation but additionally filled 28 cells that the reference leaves blank. For slide text sizes, OpenSky formatted only individual words; native left the second text box unchanged. These observations do not demonstrate a new SDK defect. The earlier intermittent Unicode typing failure remains unresolved.
