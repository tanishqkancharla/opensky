...
| Linux popup visibility (DROPDOWN-CAPTURE-01) | Merged PR9 (`3a8b719e728914adaa27e22852948a4197274e39`) includes one proven foreground same-PID/direct-transient COMBO popup in the unchanged app canvas. Shape, opacity, occlusion and metadata changes fail explicitly. Background and Wayland behavior are unchanged; pixels outside the app canvas remain excluded. | The merged full Git tree equals certified `040d0cacf`. Canonical Linux CI34594188537 passes all 129 original cases. Unchanged POPUP-L01 fails on old ed9/8b50 and passes on the canonical-certified 040/30afe binary: both popup regions exactly match all 5,734 desktop pixels. Focused CI34593382743 passes 84 unit and three real X11 controls. Postmerge installer CI34597265290 passes; exact merged identity, live configuration and owned cleanup receipts independently checked. No new agent-score or published release claim. |
