...
| Linux popup visibility (DROPDOWN-CAPTURE-01) | Candidate040d0cacf includes one proven foreground same-PID/direct-transient COMBO popup in the unchanged app canvas. Shape, opacity, occlusion and metadata changes fail explicitly. Background and Wayland behavior are unchanged; pixels outside the app canvas remain excluded. | PR9 is not merged. Same-popup POPUP-L01 c95e8a4 fails on ed9/8b50 (5,673/5,734 mismatched pixels) and passes unchanged on040/6cee: both app regions exactly match the desktop. Real X11 ownership/clipping/refusal controls pass in CI34593382743. Canonical Linux CI34594188537 and final certified-binary replay remain pending; no agent-score or release claim. |
