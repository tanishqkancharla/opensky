...
Driver-owned headless standalone Chromium is covered independently from any
in-app browser. Consumer-style SDK E2E tests are in
[../e2e/README.md](../e2e/README.md), with real-fixture prerequisites and pending
...
| Complete existing-browser inventory and provider identity (LIB-014) | Automatic enumeration now binds each exact ordinary window of matching Chrome/Edge processes, returns every CDP page as a stable session-scoped provider tab ID, reconciles open/closed tabs, and preserves user-owned cleanup protection. | Contract coverage passes for multi-window discovery, stable refresh, exact action routing, selected state, disappearance, and borrowed-close refusal. Real disposable-desktop acceptance is still required for two profiles, duplicate titles/URLs, selected-tab changes, close/reopen, and cleanup that leaves user tabs open. |
