...
| Linux popup visibility (DROPDOWN-CAPTURE-01) | Investigate app-window capture that returns a black rectangle for Calc's open validation dropdown. Return the actual visible options with proven popup ownership and stable coordinate mapping; preserve exact-window isolation. | Real SDK C02 reproduces the black popup on certified ed9/8b50. Native shows options in a separate run. First compare app/root/app captures of the same popup without intervening input, then verify owned popup rendering, closed/replaced popup invalidation, sibling windows and unrelated occluders on the exact candidate. A successful dropdown selection or diagnostic run is not screenshot acceptance. |
