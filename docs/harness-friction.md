...
- CI validation: setup-only run34696900835 passed on exact source779431738e26cdb131d5cba2c4bf0eccd1784f04. The retained executed linux-smoke.ts fingerprint matches the commit. Native screenshot readiness succeeded on its first frame, original document bytes stayed unchanged, model admission was skipped, and owned app exit plus temporary removal passed. No agent/result/score/budget artifacts were created. Evidence: work/ocr-readiness-setup-review.json. This is setup acceptance, not an agent score or proof that all timeout flakes are eliminated.
