...
The frozen evaluation build refuses native paste. This candidate routes Linux plain-text paste through the compound driver operation, preserving exact window binding and refreshing after uncertain delivery without replay. The positive consumer workflow uses the existing isolated Writer fixture: select the contextual paragraph, paste two lines, save, and assert the exact text with untouched first/last paragraphs. No mocks, clipboard substitution, injected DriverClient or expected rejection. SDK and E2E typechecks pass; no desktop run or passing acceptance is claimed yet. Full-format/race/HTML clipboard cases remain separate. This work is isolated from the frozen V3 popup campaign; its driver build and all paid outcomes remain unchanged.
