...
## LINUX-IMPRESS-NATIVE-CLICK-01 — Isolate title click and Enter across real input backends

- Symptom/layer: retained V2 font recovery inserted `Target a` / `udience` paragraphs. Public OpenSky indexed and coordinate controls both reproduced the split, while omitting Enter preserved text and saved a real bold edit. This does not establish an indexed-only bug or native equivalence.
- Prepared control: isolate from exact3560252; retain existing SHAPE-CLICK-L02 action body and add NATIVE-SHAPE-L01. New test calls actual packaged `@oai/sky` click, Return, Ctrl+A, Ctrl+B, Escape, Ctrl+S and visible-format confirmation, then reads the saved 22-slide PPTX against the explicit split-title hypothesis with all other text preserved. A different saved result fails this hypothesis and remains diagnostic evidence, not a repaired outcome.
- Coordinate fidelity: existing OpenSky fixture exposes the owned PID/XID and observed app screenshot dimensions without adding input. Native fixture reads active XID, owner PID and X11 client geometry, requires dimensions equal the app screenshot, translates the existing title point by that observed origin, independently OCRs the native 1280×900 screenshot and requires the translated point inside its title bounds. Geometry/focus must match before and after observations. It never assumes desktop and app coordinates equal. The native screenshot point is not claimed to reproduce AT-SPI indexed shape-center geometry.
- Lifecycle: reuse real native-keyboard-control RPC/error/owned-process cleanup, adding public click and returning the already-retained screenshot filename for OCR. Failed/unresolved cells poison the transport; no input replay. Native final screenshots/error logs and the existing whole-deck saved document/texts/app/temp cleanup survive test failure. Native screenshot OCR supplies the visible format-dialog assertion without OpenSky keyboard input.
- Workflow: standalone registered linux-typing.yml path, no-model repl case, exactly L02 plus native L01, one worker, bail0/retry0. Download exact ed9 release artifact34580005558 (`8b50bfbdbd9c98aaa5d0ff9d4f2196b18115b96fe6c2d68847ddb16c21ce6692`) and frozen native artifact34310637093 (`2caa7df314ce37e9048359d8e6a4a78e24574a3b54d6bf510f17754b66dda775`), hash-check before setup, verify live driver source. Install real Impress/GTK/Openbox/OCR/fonts-liberation dependencies and download only the manifest-hashed original45_1 input; record font inventory/substitutions. No model keys, reservation, score or budget inputs. Uses existing native package/runtime bootstrap; no GUI has run yet.
- Validation: full e2e TypeScript typecheck, offline YAML and shell syntax, explicit two-case selector, original L02 bytes and src/evaluator equality checked. This is draft test preparation, not runtime acceptance. Native package artifact availability, host font layout and actual saved output still require the first controlled CI run. Frozen V2 and earlier failed/control evidence remain unchanged.

### LINUX-IMPRESS-NATIVE-CLICK-01 follow-up — Explicit absolute client coordinates
