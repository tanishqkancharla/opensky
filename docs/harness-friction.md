...
### FONT-AGENT-FOCUS-01 — value replacement does not establish keyboard focus

The fresh screen-origin smoke pair (source81cdcee, driver2e81b, CI34558308462/34558911927) completed with two valid failures and matching fingerprints. OpenSky saved only `audience` at60pt and `you` at28pt; all text and other formatting stayed unchanged. Its initial unfocused setValue+Enter damaged the selected title before the agent undid it. Later indexed font-field clicks worked, but the agent did not reselect the entire textbox. Native saved the full requested60/28pt and unchanged text, but explicitly changed both font families (saved Arial instead of original Marcellus/Poppins). Original scores remain failures; no new parity pass or causal improvement is claimed.

General public API documentation now explains that setValue does not guarantee keyboard focus or send Enter, and subsequent keys target the focused control. The same description is included in the public evaluation adapter, without task-specific instructions. Frozen source81cdcee and its completed outcomes remain untouched. Retained L01/L02 and the agent trace establish the focus distinction; the SDK typecheck and six existing evaluation-adapter checks pass, with no new model or driver acceptance claimed. Pair accounting is$0.8444626; global settled$126.5172288, zero reserved, approved ceiling$150. No expansion beyond smoke until failure diagnosis.

### LINUX-CERTIFICATION-ROUTE-01 — use registered canonical workflow

The fork does register opensky-driver.yml. Dispatched desktop=all on immutable codex/linux-screen-origin-certification at exact2e81b: CI34558519742. Linux, Windows and macOS ordinary builds/unit tests pass. Linux capture has6passed/1failed/0skipped; desktop scroll reports delivery but its visible fixture offset remains0. Retained artifact10183662184 includes recordings and source identity. The exact point960,863 lies on the visible Scroll viewport target in before/after screenshots, which show no movement. Missing-marker default and nested scrolling still require investigation before product attribution. Other desktop lanes remain pending. These results do not complete canonical certification.

Separate installer bridge source d589ee8 on codex/linux-installer-certification now runs CI34559249171. First dispatch was rejected for invalid job-level runner.temp context; fixed using a first RUNNER_TEMP/GITHUB_ENV step. No model spending. Installed binary identity, daemon protocol and cleanup evidence remain pending. Review completed Linux artifacts independently of unrelated platform jobs to shorten feedback without weakening final certification.

### LINUX-INSTALL-COMPLETE-01 — installed product verified

CI34559249171 at SDK bridge d589ee8 passed. The unchanged canonical local installer builds release source2e81b, installs the actual opensky-driver executable, and the installed product reports source2e81b through identity and get_config. Artifact10183839934 was independently retrieved and checked; installed binary SHA256250a7dd15335178b23c04fb2888ea1bd101d0b290105b00ff1b12d8becc730be. Cleanup proves owned daemon exit and private installation paths removed. This is local-install-path verification of the candidate, not evidence of a published release or complete desktop certification.

The native Linux lane retains32passes/7failures in39typed cases; its report summary is absent. Retained foreground scroll observations advance content to line03/07 while scroll_offset is missing, proving an oracle visibility problem for those cases. Offscreen popover roster/actions have the same fixture setup limitation. Draft7 scopes ordinary visible setup and preserves outcome checks. A separate lost structured pixel-focus refusal is corrected in driver ce1b14809 (draft6); focused unchanged background/foreground typing CI34559870891 is active. Full Linux/Windows shared lanes remain live; no restart based on elapsed time. No model spending.

### LINUX-CERTIFICATION-FOCUSED-02 — retain missing sentinel failure

Refusal check34559870891 compiledce1b and passed foreground typing. Its background row failed during setup because the isolated workflow builtGTK3 but omitted the canonical Electron sentinel; no background input acceptance occurred. Artifact10183983358 is retained independently. SDKcheck2190e9b restores bothfixturedependencies and preflights their executables; same unchanged two rows run in34560230870. No product change or original score replacement.

Visible setup candidate4815d9030 (draft7) has parent-reviewed shared helper, original action/assertion preservation and rustfmt/diff checks. It performs at most one public X11 resize only when required controls are absent, verifies actual geometry through exact-window list_windows, and requires fresh visible labels before recording/background sentinel. Unrelated rows and Wayland unchanged. Isolated SDKcheck78d2853 runs eight affected outcomes in34560340210. Runtime acceptance pending; all new checks use zero model budget.

### LINUX-COMBINED-CERTIFICATION-02 — accepted focused fixes, full gate active

Visible-fixture run34560340210 passed all eight expected outcomes on4815d9030. Additive local checker verifies terminal exact workflow/source receipt, test hash, unique cases, fixed original oracle sets and nonempty safe video members. Independent native scroll state reads show0→84 for AX and0→161 for foreground pixels; background pixel input correctly refuses with unchanged state. Desktop-scroll immediate after.png precedes event settling, so its positive-offset assertion and full retained recording are the outcome evidence; no claim that the immediate image already shows motion. Local Homebrew ffmpeg could not decode due a missing libx265 dependency; user installation was not modified.

Prior full shared lane34558519742 contains83passing typed cases (48delivered/35expectedrefusals), matching declared cases and original oracle sets. All83video/trajectory archive members exist; this is not independent per-frame review. Original Linux native/capture failures remain retained.

Combined verification-only branch403572f34 cherry-picks exact accepted refusalce1b and fixture4815 with attribution; independent diffs confirm their product/test bytes. Wrappercd6a406 starts all three complete canonical Linux release lanes plus local installer in34561195062, with no filters and original strict preflight/video/minimum-cell requirements. It retains the release binary for future SDK/eval use after full acceptance. Full gate pending; no source or result in either frozen paid campaign changed, no new model spending.

### RELEASE-CAMPAIGN-STAGING-01 — keep exact SDK source in release acceptance

The next release campaign is isolated at codex/linux-release-campaign, initially0eaee203da74f674db60b7bbeb64de8017273378 from frozen81cdcee plus attributed9279 public focus documentation. Frozen campaigns and their failures remain unchanged. Local stage-linux-sdk-candidate.py now accepts an explicit full --sdk-source pin, retains the previouse8 default, resolves the commit, compares the entire archived src tree and records that exact source. Local default/matching/mismatch/abbreviated-pin checks pass; these are staging checks, not real-driver evidence. Three separate release templates preserve all prior fixtures, specs, actions, oracles and runners, changing only src/cua.ts comment and evals/cua-repl-tool.ts generic public documentation.

Combined403 full Linux certification34561195062 is still running. Its installer lane passed; independently retained identity/config name exact403 and owned daemon/temp cleanup is verified. The installed executable itself is not uploaded in that artifact, so its hash is a build receipt rather than an independent rehash. Full behavior lanes, shared-lane release artifact/hash, remote release SDK acceptance and fresh matched font/dropdown smoke remain pending. Budget approval reiterated this turn; settled accounting stays$126.5172288 with zero reservation under$150. No paid dispatch or local GUI work occurred.

- Combined403 certification34561195062: capture7/7 and native39/39 passed. Native comprises32delivered and7expected typed refusals. Independent inspection matches every original2e81 case ID, expected behavior and oracle set; all videos/trajectories exist, no failures/skips. Source identities match403. This is canonical deterministic evidence, not a new agent score or exhaustive frame-by-frame review. Shared lane remains live. Active Linux plan/README now record the completed20, both-failed font smoke and actual release gate; no frozen source or score change. Remote preflight shows no running containers, matching full-desktop/font-probe images and5GBfree.

### CANONICAL-ARTIFACT-INSPECTION-01 — reuse retained outcome verification

Added local read-only work/inspect-linux-canonical-artifact.py to replace repeated inline per-lane checks. It validates exact run/wrapper/job identity, terminal selected-lane success, ready driver source, complete unique original declarations/results, expected delivery/refusal including original allowed refusal codes, all original oracle sets, safe nonempty videos and finalized trajectories. Overall in-progress workflow does not prevent accepting an independently completed lane; that lane is never promoted to full release acceptance. Native39 and capture7 pass against retained current403/CI34561195062 evidence, with exclusive additive reports and exact CI receipt copies. Oldfailednative rejects; one temporary wrong-refusal-code negative control rejects despite otherwise passing typed results. Temporary controls removed, original bytes preserved. The tool does not decode videos, regrade, dispatch, or replace canonical tests. Shared lane remains active; spending unchanged.

- Combined403 full Linux certification34561195062 finished successfully: shared83 (48delivered/35expectedrefusals), native39 and capture7, no failed/skipped canonical cases; installer identity/config/owned cleanup pass. Independently checked all original contracts/oracles, refusal codes, finalized recording paths and source/CI identities. Downloaded release hash e65549f1a2d77d168581a2c2cc84585f90b51748a1cf85c2cf43bfca7a49ba85 matches the installer hash and shared build receipt. Full shared recording retrieval244MB delayed results; separate workflow-only3187c58 now adds small metadata artifacts for future runs without changing original artifacts/scripts. Syntax checked, not CI-replayed.
- Release SDK acceptance: release-font-403-after-01 passes four selectedL02–L05, all saved60pt/Target audience and owned app/temp/container cleanup; release-code-403-after-01 passes exact Updated text\nKeep this line\n at displaced geometry with cleanup; release-calc-403-after-01 passes CALC-L01/L02,FOCUS-L04,CLICK-L03 with independently saved E2=B2-C2/75000,E50=42,28Pass/Fail/Held rules/visiblearrows/help titles, unchanged original values and target/sibling documents. All five app groups exited and container inventory empty. Focus has explicit temporaryRemoved; Calc/click rely on successful fixture teardown and container cleanup, not nonexistent separate temp receipts. All nine use SDK0ea and exact403release.
- New frozen paid source2d8879f differs from staged0ea only by workflow binary/source admission and friction notes; runtime/evaluator/assets unchanged. Both scoring preflights pass. Freshfont pair controller19160 dispatched OpenSky34563990218, reservation977bada7-1b86-4bd1-a151-659fd246ed87 under existing$150cap; settled$126.5172288+$5reserved. Native follows automatically after first terminal evidence. Live scorecard preserves20-task16/14 and adds current font comparison narrative, verified through status-release-font-start.json. No new agent success claimed yet.

### MERGED-LINUX-01 — landing verified driver changes

User explicitly authorized merging driver PRs5–7 and pushing to the fork after automatic review blocked publishing prepared summaries. Summaries were published and all three merged normally into codex/linux-observation-visibility, final e25f348b3e0ca9221130e1df8e792ca857cc0505. Parent independently verified all merged API states and git tree: only docs/linux-screen-origin.md differs from certified403. Frozen certification branch2e81 and paid source2d remain unchanged. Canonical private-path release installer smoke34565230161 on wrapper c0a754c is pending; prior129canonical and9SDK results retain exact403 attribution. No other PRs, main release, force push or local GUI action.

Fresh release smoke pairs both pass: font OpenSky62.822s/10calls/$0.3480744 and native82.713s/25calls/$0.6438062; dropdown native41.036s/12calls/$0.3210988 and OpenSky65.158s/11calls/$0.4911426. Pair fingerprints match, all app/temp cleanup and usage verified. Independent saved outputs show full60/28pt with original font families/text and exact requested dropdown validation with unchanged cell values. Previous failed campaigns remain unchanged. Declared five-task expansion adds Writer strike, Code replace-all and Calc profit on exact2d/403. Settled$128.3213508 before expansion; existing$150checkpoint remains enforced per serial reservation. Single attempts do not establish statistical parity or release-only causality.

- Post-merge canonical installer CI34565230161 completed success on wrapper c0a754c and exact driver e25f348. Independent retained identity/config confirm product opensky-driver, protocol1, version0.23.2 and source e25f348; private install link, daemonStarted/verifiedExited/temporaryRemoved=true and exitCode0 verified. Runner hash receipt c72954daf6752c48c139604b33c3b2bddf49115183346de16d98990b2130e6f9 retained; installed binary itself was not uploaded, so no independent local binary rehash claim. No GUI task or main release implied.
- Release OpenSky strikethrough CI34565367524 passed45.894s/8calls/$0.265384. Saved document has all387last-paragraph characters struck, unchanged text and other paragraph strike state. Cleanup/usage verified; native06 active.

- Native release strikethrough CI34565606078 is a valid failure28.256s/6calls/$0.209915: only13of387last-paragraph characters struck. Text and otherparagraphs unchanged; first failure, cleanup and matched fingerprints retained. Three pairs now OpenSky3/native2; Code replace pair07/08 begins on unchanged2d.
- RELEASE-TAIL-01: old visibility tail hardcodes the earlier source and different firstfive; invoking it would repeat font/editor tasks and omit remaining subscript/freeze tasks. Prepared separate work/run-release-full20-tail.py, binding exact2d/403, original20membership, retained five-selection/evidence hashes and existing per-pair settlement/fingerprint validation. Same scoped source check as dispatcher tolerates dependency symlinks but refuses dirty runtime/evaluator files. Parent reviewed syntax and source scope and revalidated three actual retainedpairs, including the native failure. No full20selection adopted or tail run yet. Existing dispatcher remains sole budget/reservation owner.

- Release native Code replace CI34565896643 passes13.850s/3calls/$0.1553952. Saved5-line text exactly matches gold and literal replacement of all10text occurrences, including trailing newline. Finalusage and ownedapp/temp cleanup verified; OpenSky08 follows without source changes.

- Release OpenSky Code replace CI34566131694 passes26.403s/7calls/$0.3356604. Saved text exactly matches gold/literal10replacements/trailing newline; paired native fingerprints and allcleanup/usage match. Four matchedpairs now OpenSky4/native3. Final five-task profit pair09/10 starts on frozen2d/403; settled$129.2877054 before its reservation.

- RELEASE-ADOPT-01: prepared work/adopt-release-full20.py to build the immutable remaining15plan only after allfive actualpairs validate. Parent inspected code and actual firstfour pairs; profit correctly pending,40uniquearms and originalordering verified. Explicit--adopt writes exclusively; default has no campaign mutations/model calls. Firstfive selection+original20ordering are hash-pinned, allretained outcome/admission/cleanup/usage/environment/file hashes preserved, validnativefailure retained. No full20selection adopted yet.
- Live scorecard now selects freshreleasefive (4matchedpairs,OpenSky4/native3); previous20configuration retained separately. Actual /status.json verified stage5,planned5,completed4 and ledger$129.2877054+$5reserved/$150. Historical20scores remain unchanged, and internal installer work is excluded from the canvas.

- Release OpenSky profit CI34566409226 passes142.567s/16calls/$0.8150014. Independently read cachedJ2:J10andYear_Profit summary values allmatch gold; noextra nonempty/formula cells. Sheet order differs fromgold but is not requested or part of the frozen named-sheetcontent scoring. Cleanup/usage verified; native10 starts, settled$130.1027068. AllfiveOpenSky tasks passed, but onlyfour matchedpairs complete untilnativeprofit finishes.

- Release native profit CI34566720738 passes51.292s/13calls/$0.3901516, with all9profit values/yearprofit summary and noextra nonempty/formula cells. Matched environment and allcleanup/usage verified. Five-task rampcomplete OpenSky5/native4; native strikefailure retained. Parent adopted full20-selection hash1bc34dae after allfivevalidated, then dryran15remainingrows and launched serialtail21965 on exact2d/403. Settled$130.4928584 before tail; cap150unchanged.
- INSTALL-BRANCH-01: SDK README omitted a driver branch, while forkdefaultbranch main contains the upstream installer. Verified authoritative default and exact merged wrapper; documented branch plus pinned e25f348 checkout and releaseinstall for Linux. This routes users to the already tested OpenSky build; does not execute an installation locally, move a fork branch, change the frozen campaign or claim macOS/Windows acceptance. Exact merged installer34565230161 covers the delegated install-local path; syntax/docs diff checked.

- Release native fill CI34567021965 passes43.020s/4calls/$0.2007712. All46changes are previouslyblank B1:E30 cells, complete savedvalues match gold and sheetorder preserved. Cleanup/usage verified; OpenSky12 active. Diagnostic invocation map prepared and references/assets checked for allremaining15tasks; fill requires explicitcurrentcampaign, subscriptrequiresfinalDOCXpath and priorfinalization.
- Interim completion audit rechecked native paste implementation and staticTODO declarations: public non-browser paste remains an unconditional refusal; native clipboard full-format/race/HTML and existing-profile/tab identity acceptance remain missing. Frozen20Office/editor success cannot prove these APIs. Newer Linuxselection/focus/error tests must be matched to exact originalcontracts before closing their TODOs. Preserve broadgap visibility and keep currentcampaign frozen; this audit did not change scoring or executeGUI.

### FILL-L01 / FILL-L02 — Release fill failure and preservation replay

The frozen release comparison now has seven completed pairs: OpenSky 6/7 and native 6/7. Both passed sorting with all 18 records and headers preserved. The newly retained OpenSky fill failure (`12-calc-fill-opensky`, CI `34567226297`) changed 50 cells instead of the requested 46 blanks: C12, C16, C22 and C25 were overwritten with values intended for preceding blank cells. Native fill passed. The original run and failed score remain unchanged. Retained AX/agent traces show that none of the four populated cells was explicitly targeted, and corruption appears before save/correction. Pointer targeting versus edit/commit sequencing remains unproven.

A separate test branch (`codex/linux-fill-preservation`, `a120972`) adds a two-cell indexed replay and visible-name-box control through the public SDK, with decoded whole-workbook comparisons covering every untargeted cell and sheet order. Source runtime is unchanged from `2d8879f`; the staged driver is exact `403572f`/`e65549f…`. Local typecheck and comparison against the retained failure passed; real remote results are pending. These tests import the public API from source and do not certify package installation. Native paste remains a separate unimplemented gap, with its positive test drafted on `codex/linux-native-paste` (`0675cc9`).

### INFRA-L15 — Benchmark host unavailable before agent admission

Chart arm `15-calc-chart-opensky`, CI `34568135736`, failed during asset installation after three HTTP 503 responses for an unrelated presentation asset. The authoritative job shows reservation admission and isolated agent execution both skipped. No result artifact or behavioral outcome exists; the serial controller stopped without redispatch and retained its reservation. Recovery must preserve this failed infrastructure attempt and the immutable source/task/grader, using the existing spending admission rules. It must not replace an actual evaluated failure or claim the absent chart run passed. Recovery is pending.

The no-model `calc-fill-replay-before-01` now reproduces FILL-L01: C10 is written, C11 remains blank and C12 is overwritten. FILL-L02's visible-name-box control changes only C10/C11. Both real owned app groups exited and the fetch confirms an empty Docker inventory. Evidence is retained under `evals/runs/linux-office-setup/calc-fill-replay-before-01`; no product fix has been claimed.

INFRA-L15 recovery completed with fresh authoritative attempt-1 CI/job/log proof. The original dispatch and failure evidence are archived with hashes in `15-calc-chart-opensky-pre-admission-34568135736`. The existing budget API settled the unused reservation at zero; only then was a new reservation issued for the same immutable arm/source/task/model. Replacement CI `34568823330` is running at attempt 1. No failed agent outcome was replaced, and the remote anti-replay guard remains unchanged.

The current frozen release campaign has nine completed pairs: OpenSky7/9, native6/9. Chart passed for OpenSky; native added33 data cells to the chart sheet. Split-name output failed on one cell for OpenSky (Eastwood instead of East), nine for native. All original grades, usage and cleanup receipts remain retained. OpenSky presentation-background arm19 is running; settled accounting is $134.953078 with $5 reserved under the $150 cap.

The fill branch now preserves the no-probe regression and several bounded diagnostics. Actual screenshots prove an indexed C11 click selects C12 before typing. Cell identity/Position remain unchanged; table and cell Screen coordinates shift24px after the first edit while Window coordinates remain unchanged, and the current17px rebase then misroutes. Parent-table hit testing shares the initial stale coordinates, so it is not an independent physical oracle and no chooser was implemented. Next investigate the table's advertised single-cell Selection behavior, including selection multiplicity and keyboard focus, before proposing a product fix. Original no-probe FILL-L01 must pass through the real public SDK for acceptance. A new empty driver worktree is based on merged e25; no driver product change or new driver PR has been made for this issue.

### CALC-SELECTION-EXPERIMENT-01 — test one semantic cell selection

The existing wrong-row regression has a scoped driver draft, tanishqkancharla/cua#8. Candidate30189c6 uses TableCell Position, its owning table's child index, exact retained identity and one SelectChild call, then exclusive selection readback. Any uncertain mutation terminates without pointer replay. It is an experiment: hosted release compilation/focused checks run34571597335 is in progress; unchanged public-SDK FILL-L01/L02 saved-workbook validation is next. Missing interfaces, merged cells and an existing multi-selection remain explicit promotion risks. No frozen agent runtime was changed.

The remote VM has no Rust builder image or Cargo volume, so a fresh remote toolchain would slow this iteration. The isolated hosted wrapper12fdd11 pins the exact candidate and release build. Original SDK source2d, original full-desktop image and original two-test replay template will be retained.

Current release campaign has eleven matched tasks: OpenSky9/11, native8/11. Both systems pass background and table-header saved outcomes. OpenSky23 also passes duplication (26 slides, final A/B/A/B order); only surrounding trailing whitespace changes appear in earlier slides21/22, and export geometry differences remain separately reported. Native24 is running. All completed new arms have owned cleanup and final usage receipts; original failures remain.

### CALC-GEOMETRY-01 — moved window remains a counterexample

The SelectChild candidate30189c6 failed by marking cells while typing stayed in A1. GrabFocus15adb passed FILL-L01/L02 but failed name-box focus transfer and existing range selection. Both are retained and superseded by geometry-only d3005c0c3, built in CI34574603781 with78 focused checks. Its exact binary b1ca737e5fa6c20991efe95dd77e5845ec6aaf93ba827c4c737b03c59bb6aca0 passes FILL-L01–L05 in remote `calc-geometry-six-after-01`: only targeted cells change, merged ranges remain intact. FILL-L06 fails when restored/moved to client(201,118),960×700: C9/C11 change instead of C10/C11. All six owned groups exited, full saved workbooks were independently read and container inventory is empty. Draft driver PR8 remains unaccepted; a separate coordinate diagnostic is prepared on SDK test commitc66b050. Source/runtime/scoring in the paid campaign remain frozen.

Current release score is13 paired tasks: OpenSky11/13, native10/13; OpenSky27 indentation passed but native28 CI34573099694 remains in its combined desktop/scorer install step before model admission. CLI and job-log REST cannot expose its logs until completion (REST404 BlobNotFound); the specific blocked command is unproven. Future improvement: separate setup into named steps and use content-verified dependency/asset caches. Do not change or restart a frozen run solely because installation is slow. Budget137.69024 settled plus5 reserved under150.

Native28 completed and passed: exact ten-line saved Python text matches gold, including indentation and final newline;20.184s/four calls/$0.1727132 with verified owned cleanup. The completed pair has identical fingerprints and brings the score to OpenSky12/14, native11/14. Native29 document centering CI34575793577 is active; settled137.8629532 plus5 reserved. Terminal setup logs resolve the slowdown: apt fetched145MB in31m12s from the Azure Ubuntu mirror. Code completed in~1.4s; the earlier possible Code-stall explanation is disproved for this run. Named setup steps and exact-package caching remain future campaign improvements.

CALC-GEOMETRY-01 diagnostic `calc-moved-diagnostic-after-01` failed before edits because the local archive omitted calc-cell-identity-probe.py. Preserve it as staging infrastructure evidence. The corrected exclusive v2 archive adds that exact committed probe and retains the same source, image and test; no acceptance result is implied.

### CALC-MOTION-01 — first pointer input changes LibreOffice screen origin

Diagnostic `calc-moved-diagnostic-after-03` now retains every screenshot. The first indexed C10 click visibly selects C9 before typing; after Enter, C10 is selected; the second indexed click selects C11. Same-driver raw probes retain identical cell identities/Window extents while Screen Y changes24. The certified403 baseline reproduces the same wrong saved C9/C11 under the identical diagnostic template/image in `calc-moved-diagnostic-before-01`, proving this case predates d300. Both runs have independently checked sources, saved files and owned cleanup; no containers remain. Exact LibreOffice24.2.7.2 source links the shift to its first pointer event replacing top-level configure coordinates with event-box coordinates beneath the menu. Next isolate motion before the button; do not substitute an arbitrary24px correction.

Frozen release lowercasing pair passed for both systems, bringing matched outcomes to OpenSky14/16 and native13/16. First failures and matched fingerprints remain unchanged; all32 completed arms have final usage and owned cleanup. Native33 font change CI34576758750 is active; accounting138.6828822 settled plus5 reserved under150.

CALC-MOTION-01 control passed in `calc-motion-diagnostic-after-01`: pointer motion alone changed C10 Screen Y425→449 before the first button/key, and the saved workbook changed only C10/C11. Window extents/identity stayed fixed. The X11 child tree exposes only1×1, so it cannot provide the input widget inset. Exact source/hash/image, six screenshots and owned cleanup were retained. Driver candidatec98a379c8 adds a guarded pointer pre-position then fresh same-object bounds, using token/name/role/liveness checks and non-replay errors after motion. A50ms native settling interval is not an app acknowledgment; asynchronous layout/focus races remain documented. CI34577807466 is compiling; unchanged six public-SDK cases remain required before acceptance.

Both font-change arms passed, bringing the frozen score to OpenSky15/17 and native14/17. All34 completed arms retain final usage/cleanup and saved outcomes; OpenSky35 subscripts CI34577471774 is active, accounting139.1852998 settled plus5 reserved under150.

CALC-MOTION-01 candidate ed9fd15e3 fixes the pointer-refresh callback's retained runtime scope (caught before product acceptance). CI34578176533 passed; binary261eb99a1d1687fc3c01cc0c8a2631bf3f593b933b98e941bc1b3f04d8cfa460 reports exact ed9/v0.23.2/protocol1. Remote calc-pointer-six-after-01 passes all six unchanged FILL-L01–L06 cases on the original image and SDK2d. Independent whole-workbook inspection verifies only requested edits, all merges preserved, moved-window geometry retained and all six owned groups exited; container inventory is empty. Existing Calc/Code/font controls and exact-candidate canonical certification remain pending. No paid runtime or score changed.

Release subscript OpenSky35 is a retained valid failure: all text unchanged, zero of eight requested digits subscripted; six O letters and two x letters received formatting. Saved formatting and successful toolbar calls establish the wrong selected characters, not an unsaved-file failure. The exact four-arrow sequence differs from an earlier five-arrow native control, so driver versus selection-workflow attribution remains unproven. Native36 CI34577947376 is still installing desktop/scorer dependencies; paired score remains OpenSky15/17 and native14/17. Accounting139.6384824 settled plus5 reserved under150.

CALC-MOTION-01 ed9 regression controls: calc-pointer-calc-after-01 passes CALC-L01/L02, CLICK-L03 and FOCUS-L04; saved formula/value, validation range/title/choices, unchanged sheet values and both focus documents independently verified. calc-pointer-code-after-01 passes displaced editor replacement with exact saved two-line text. calc-pointer-font-after-01 omitted its intended filter, running all7: L02–L05 required controls and raw-center L07 pass; historical unfocused-setValue L01 and deliberately rebased coordinate L06 fail (saved text empty / size70 respectively). Preserve all7 and do not claim a green suite. All owned cleanup verified. Thus15 intended SDK checks pass including six cell-preservation cases. Exact-candidate canonical Linux/installer CI34580005558 is running from wrapperd9f5a3f, draftPR8 remains unready.

SUBSCRIPT35-D01: exact frozen2d/403 two-case remote subscript35-403-before-01 passes its diagnostic predictions. Independent ODT read shows direct selectText2 with H/O context replaces only2, while selectTextH2O plus observed Right,Left,Left,ShiftRight replacesO. Surrounding paragraphs unchanged; both app groups exited and temporary directories removed. NativeLinux source matching the retained client fingerprint exposes noselectText API, so ordinaryFind cannot silently substitute as identical initialselection. Driver-versus-workflow attribution remains unresolved; original paid failures remain. Native36 subscript failed with1of8requested digits formatted, text unchanged;18matchedtasks OS15/native14. Native37 sheetcopy passed exact requested sheet order and all sparse values,48.118s/14calls/$0.350119; OS38 CI34580034264 active. Accounting140.161827settled+5reserved/150.

### RELEASE-20 — completed frozen Linux comparison

All 20 matched Terra-medium pairs are complete on unchanged SDK `2d8879f` and driver `403572f`: OpenSky 17/20, native 15/20. Fourteen tasks passed on both, three only on OpenSky, one only on native, and two failed on both. The only native-success/OpenSky-failure task is spreadsheet blank filling. All pair fingerprints match; all 40 completed arms retain final usage, saved-file diagnostics, and owned cleanup. The initial five and every first failure remain unchanged. OpenSky freeze panes passed with the requested row and two columns frozen and workbook formulas/values preserved; native froze only the row. Cumulative conservative evaluation accounting is $141.1216626, with no reservations. Controller 10805 finished; no new paid campaign is running.

The live page now shows the completed baseline and the next behavior to fix. This is one run per backend on a frozen 20-task OSWorld subset, not a statistical or full-API parity proof. Candidate `ed9` remains separate: 15 intended SDK controls pass; canonical capture 7/7 and native 39/39 plus installer are independently verified; shared 83-case certification is pending. After certification/merge and postmerge smoke, measure the blank-filling fix in a new small paired campaign. Native paste, clipboard restoration, and other unverified API contracts remain explicit gaps.

### CALC-MOTION-01 — certified and merged

Driver PR8 merged at `53b2a81deaa7119a31a0e68bbfc0962e1652bf90`. Its complete Git tree equals certified candidate `ed9fd15e39a1d2acd07c4f3078440c3ab2ad44f2`; no executable or harness changes were introduced by merge. CI34580005558 passed all 129 original Linux cases (shared83, native39, capture7), with no failure/skip/case-definition change. Independent checks retained all typed oracle/refusal evidence and full recordings; recordings were not exhaustively frame-reviewed. Fifteen intended SDK cases and the local installer passed. Certified release binary `8b50bfb…` differs from the separately built SDK-test binary `261eb99a…`, both at exact ed9; each hash is recorded independently.

A local orchestration guard expected GitHub merge state CLEAN but observed UNSTABLE; the subsequent merge tool call was not conditioned on that command's nonzero exit. Exact head matching still constrained the merge to ed9, and release metadata checks passed before readiness and in postmerge reinspection. Record this guard failure rather than calling it a clean premerge check. Future dependent mutations must execute only after inspecting the preceding command's exit status, not merely follow it sequentially. The merge tree is independently verified unchanged. The fork's postmerge auto-release job34582804240 failed at missing app-id while generating a GitHub App token; this does not invalidate driver tests, and no published release is claimed. A warm existing installer wrapper now pins exact merged53b for the short postmerge smoke.

Postmerge installer CI34583073298 completed successfully on exact merged `53b2a81`. Identity, canonical installation/symlink, configuration, daemon start and verified exit/temp cleanup were independently inspected. The headless daemon log retains expected missing-display/AT-SPI warnings; this smoke establishes source installation, not desktop or release distribution. The separately frozen correction wrapper `fcd01bb0` now runs blank filling as a new one-task pair (`campaign-linux-calc-pointer-candidate`), preserving all 40 completed baseline arms. OpenSky first run CI34583825229 has been admitted; native follows only after settlement under the unchanged $150 checkpoint.

### CALC-REFRESH-01 — successful smoke retains index failures

The new paired blank-fill smoke completed on exact fcd01bb0/ed9: OpenSky118.074s14calls; native97.797s8calls. Both saved exactly46correctblankfills with allothercellvalues and sheetorder preserved; matching environment receipts and cleanup passed. OpenSky recovered from two freshly advertised B3index refusals using screenshot coordinates. Thus this is a saved-task success, not proof of full index correctness or causality. Debug-enabled public SDK REFRESH-L01/L02 now isolate cleanB3 versus edit/fill-down/Undo/reobserveB3 on an unchangedruntime. The test branch379d673 is pushed; remote acceptance remains pending.

The SDK stager previously named binaries by sourceSHA alone, colliding when separate builds ofed9 had different bytes (261eb99aSDKbuild versus8b50bfbcertifiedbuild). New stages include both source and binaryhash in the immutable remote filename. Existing stages/binaries remain unchanged; local stagingv2 passed source/asset/hash and shell checks, and exact8b50binary/stageuploaded without replacing261. The initialv1 was never uploaded or run.

Five-task selection preserves the completed smoke pair hash and predeclares grossprofit,dropdown,font,andCode replacement before dispatch. Existing pair helper owns serial reservations, outcomes and recovery; older release-full20 helpers remain hardcoded to2d/403 and must not be used for thiscampaign. Current controller starts nativeprofitCI34584930624 under unchanged150cap.

REFRESH-L01/L02 both passed in remote `calc-refreshed-index-ed9-before-01`: independent saved-file inspection found onlyB3=Primary in each workbook, exact certified8b50binary/source and both owned-app cleanup verified; Docker inventory was empty. Debug logs contained no child timeout/failure or traversal truncation. This did not reproduce the CI failure; Undo alone is not established as the cause. Preserve the original agent trace and do not convert these two passes into a claim that the intermittent index rejection is fixed. A hosted-CI replay using the same binary is a possible next discriminator.

Hosted CI34585926656 also passed both controls with exact source/binary/runtime, saved-workbook preservation and owned cleanup independently checked. REFRESH-L03 then retained the agent's original formula-toolbar panel interaction. Its first run34586872442 failed before any click because the fixture had primed diffing and the test expected a full tree from the default observation. The corrected test explicitly requests a full initial observation; CI34587521656 at d264856 is pending. Neither result establishes that the intermittent B3 rejection is fixed.

### LINUX-FIVE-CALC-02 — three pairs complete; separate task scores from failure attribution

The frozen fcd01bb0/ed9 campaign has three of five pairs complete. Both agents passed blank filling and gross profit; both failed the original dropdown scorer. Exact environment/model/scorer comparisons match and all six arms have settled final usage, saved artifacts and verified cleanup. OpenSky therefore has 2/3 task successes and native has 2/3; do not combine these with the separate completed 17/20 versus 15/20 campaign.

Both dropdown agents prefilled D2:D29, while the frozen reference leaves those cells blank. The task's wording permits ambiguity about filling that relevant column; this is a scorer mismatch to retain, not evidence that both APIs failed to create validation. Native saved the exact three-choice rule. OpenSky saved a fourth empty choice: its trace explicitly presses End then Enter inside the multiline Entries editor, appending a blank line before OK. This supports an agent editing mistake, not a demonstrated driver regression. Earlier OpenSky attempts to type the valid choice Pass were rejected even though dropdown selection worked. The trace did not capture the live editor before Enter; Calc AutoInput completion to the existing Pass/Fail/Held header is an unverified alternative explanation. New consumer controls will inspect that state without disabling normal AutoInput or changing expectations.

Paid execution stopped before arm07's reservation at $145.2964082 settled, no reserved funds, under the $150 checkpoint. Approval through $200 is pending; only the remaining font and Code pairs may resume after authorization. The existing first outcomes, selections and campaign fingerprints remain immutable.

### DROPDOWN-CONSUMER-01 — distinguish AutoInput rejection from text delivery

Remote `dropdown-consumer-ed9-before-01` at test source a6b54dc and certified ed9/8b50 reproduced the earlier invalid-value alert. C01's full screenshot before Enter shows `Pass/Fail/Held` in D4 with `/Fail/Held` selected; its AX paragraph matches. Calc completed the typed prefix from the column header, so Enter submitted a value outside the validation list. C01 stops before saving, preserving the original workbook. C02 selected Pass from the dropdown and saved only D2=Fail, D3=Fail, D4=Pass, with all 28 exact three-choice validation rules retained. Source hashes, saved artifacts, owned-app cleanup and empty container inventory were independently checked. Retain the original failed consumer test and agent score.

New C03 at cae3ac7, run `dropdown-consumer-backspace-ed9-before-01`, uses public Backspace to remove the selected completion suffix before Enter. It passed; independently inspected full AX and screenshot show Pass, and the saved workbook has exactly the same three intended cell changes and exact rules. AutoInput remained enabled and production runtime stayed unchanged. This establishes the ordinary consumer action needed for this reproduction; it does not justify a text-delivery driver patch. A matched native Fail/Fail/Pass control has not run, so these results do not establish identical native and OpenSky AutoInput behavior.

C02 also reproduced a black rectangle where the dropdown options should appear in OpenSky's app screenshot. A retained native screenshot from a different run shows the options. The Linux window capture reads the exact XID without composing separate popup windows, a mechanism candidate requiring a same-popup comparison. D01 at33b4e9 brackets an external root capture with two public app screenshots and records window posture without intervening input. Its preflight found ImageMagick absent in the original disposable image, before any GUI execution; a separate diagnostic image is being prepared. Do not treat diagnostic completion or cross-run pictures as capture-parity acceptance.


### DROPDOWN-POPUP-PIXELS-01 — proven observation gap and candidate fix

D01 and D02 are complete. The app/root/app bracket confirmed the black app popup while the desktop showed Pass/Fail/Held. D02 established a viewable override-redirect COMBO with the same PID and WM_TRANSIENT_FOR pointing to the exact requested frame. Active identity and app geometry stayed fixed; popup height settled71→61, which remains a sequential-capture limitation.

Public SDK test POPUP-L01, sourcec95e8a4, compares both app screenshots against the independent desktop using actual owned metadata and the common visible region. The old certified ed9/8b50 run dropdown-capture-ed9-before-01 failed:5,673 of5,734 pixels differed in both images. The unchanged test/source/runner/image passed with candidate040d0cacf/binary6cee00d277db in dropdown-capture-040-after-01. Independent Pillow inspection found exact RGB equality in both popup regions; parent visually inspected all three readable choices. Workbook/source/image hashes, owned-app exit and empty container inventory were verified. Each run retains independent-popup-acceptance-check.json under evals/runs/linux-office-setup. No scoring tolerance was changed.

Driver PR9 implements direct-XID composition for this narrow proven-owner case, clipped to the existing app canvas. Background/Wayland remain unchanged; unsupported shape/opacity/occlusion or unstable metadata cause explicit errors. Focused CI34593382743 passes84 unit checks (six new popup checks), three real X11 controls and release build. Original failures remain:34591610618 found a Rust borrow-lifetime error;34592310556 passed compilation/units but failed fixture activation because the intended Openbox dependency edit had not applied. The corrected workflow explicitly installs/checks the desktop, preserves WM logs and failed-build caches, and tests the live fixture before release optimization. No driver/test relaxation was needed for the second failure.

Final canonical Linux CI34594188537 is running on exact040. Certification and postmerge smoke are still required; the new candidate is not merged or distributed. Paid campaign metrics remain3/5 pairs complete,2/3 each, and the$150 checkpoint remains in force. Public screenshot acceptance is not an agent task pass.

REFRESH-L03 correction d264856 also completed successfully in CI34587521656: onlyB3=Primary was saved, with exact identity and owned cleanup verified. This still does not reproduce or resolve the original intermittent agent B3 rejection.


### DROPDOWN-CAPTURE-01 — merged and canonical acceptance complete (2026-09-11)

PR9 is merged into the fork integration branch `codex/linux-observation-visibility` as `3a8b719e728914adaa27e22852948a4197274e39`; its complete Git tree equals tested `040d0cacfcaf32e6051b372e2e1e38c2e9c4a22f`. Canonical CI34594188537 passes all 129 original cases (83 shared, 39 native-app, seven capture) and the source-install smoke. Independent artifact checks verify cases, oracles, source and evidence; videos are retained but were not exhaustively frame-reviewed.

Canonical-certified binary SHA256 `30afe80f94d32b2e53b6df6950eba6bb74c31b0382462822109fc4b70bdb001d` also passed the unchanged public SDK POPUP-L01 in `dropdown-capture-certified-040-after-01`. Both app screenshots exactly match all 5,734 pixels in the common visible popup region of the independent desktop. Test/source/runner/image/thresholds remain fixed; original workbook and owned cleanup are independently checked. The focused build `6cee00d277db` is separate and also passes normal editing C03: only D2=Fail, D3=Fail, D4=Pass change and the exact three-choice rule covers D2:D29.

Postmerge installer CI34597265290 passes in 6m24s on SDK wrapper `7cbb73da22fbb3f41bbe564ecaa9b7565377c1ca`. Independent inspection confirms merged source identity, live Linux configuration, installation paths, daemon exit and temporary cleanup. The installed SHA256 `7ddce18ba62eb9072876875797985faa228bb7963e34446707c2544360556930` is a CI receipt, not independently downloaded binary bytes. The reusable local verifier `work/inspect-linux-installer-artifact.py` reduces repeated checking and rejects a mismatched source against retained evidence. Scope and sequential-capture limitations remain unchanged. This fixes the observed screenshot omission, not the frozen dropdown task score or the unresolved intermittent B3 index refusal. Paid campaign remains 3/5 pairs complete, 2/3 each; spend is $145.2964082 with zero reserved against the $150 checkpoint.
