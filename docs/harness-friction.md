...
| LIB-014 | The first native-style facade lacked current Browser-versus-Tab ergonomics and could invite invalid calls or unnecessary observations. | Bound `Browser.tabs.new/get/list/selected`, `nameSession`, exact `Tab.close`, installed-provider discovery, top-level known-URL shortcut, explicit existing-tab attachment, headless owned tabs, and pre-dispatch argument validation. Inventories are explicitly facade-bound-only; unsupported capabilities throw. | **Contract-confirmed**, with invalid-argument **boundary confirmation** against the real helper in epoch 27 and headless same-tab **live confirmation** in SET-009. See [`src/cua.ts`](../src/cua.ts), [`test/cua.test.ts`](../test/cua.test.ts), and [`test/typed-browser.test.ts`](../test/typed-browser.test.ts). Automatic enumeration of unbound user tabs remains unsupported. |
...
| CLN-005 | “No tabs” was presented as absence of all browser siblings although the facade inventories only tabs explicitly created or attached through it. | `getState()` publishes `tabInventoryScope:"facade-bound-only"`; reviews do not infer anything about unbound user tabs. Owned and borrowed bindings remain distinct, and cleanup refuses to close borrowed tabs. | The scoped inventory and borrowed-close refusal are **contract-confirmed** by [`test/cua.test.ts`](../test/cua.test.ts); exact existing-tab detach is covered by [`test/typed-browser.test.ts`](../test/typed-browser.test.ts). Full automatic user-tab inventory remains unsupported. |
