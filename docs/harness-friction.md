...
Maintained: 2026-09-15. This is the evidence ledger for friction found while comparing OpenSky/Pi with native Computer Use. It records fixes, safety workarounds, and open gaps; it is not a parity claim.
...
**LIB-026 — duplicate native control labels could silently retarget a public
index:** a September 15 real Chrome app probe used a locally served dynamic
settings page with six identically named `Expand` buttons. Native Computer Use
removed and saved only the requested final channel. Before the OpenSky change,
opening `Default Channels` and observing again changed the retained `Name
Display` index 57 into the current `Email Display` index 57; using it succeeded
and visibly opened Email instead of Name. The SDK had greedily paired duplicate
role/label identities in order, so inserting `Close` shifted every later
`Expand`. OpenSky now preserves public indices only for identities unique in
both snapshots; ambiguous repeated controls receive fresh indices and retained
ones fail closed. The identical after-probe rejected the old index before input,
kept Name and Email closed, and a separately refreshed OpenSky flow removed and
saved exactly the requested channel while leaving the neighboring channel.
This is **live-confirmed on macOS Chrome with driver 0.23.2
(`622465b352590db3f983490261e9260537cc52dd`)** and contract-covered by
[`test/opensky.test.ts`](../test/opensky.test.ts). The tradeoff is index churn for
ambiguous controls: callers must resolve them from the latest observation. The
separate equal-count projection discrepancy is addressed in LIB-027.

**LIB-027 — presentation filtering looked like incomplete native collection:**
the same real Chrome fixture initially emitted `Accessibility projection` after
returning 325 of 325 discovered elements and performed a second shallow driver
read, unlike native Computer Use's direct current-state observation. The first
correction still reproduced the warning because it compared the driver's
returned count with OpenSky's post-filter list; pruning closed browser menus was
mistaken for unrepresented traversal data. Native snapshots now retain the raw
structured-element count before content/menu filtering. A shallow recollection
runs only when total exceeds returned, or returned exceeds that raw structured
count on non-Linux targets. The real driver now returns all six fixture controls
without the projection warning and the complete remove/save flow still passes.
The fixture regression is **contract-confirmed**, and the retained equal-count
GitHub replay still consumes its shallow projection because 1,473 returned nodes
had only 11 structured elements. The Chrome outcome is **live-confirmed on
driver 0.23.2**. This does not reinterpret `elements_complete=false` as a generic
claim of complete platform traversal; it only avoids replacing a snapshot when
every returned structured element is already represented.
...
| LIB-014 | The first native-style facade lacked current Browser-versus-Tab ergonomics and could invite invalid calls or unnecessary observations. | Bound `Browser.tabs.new/get/list/selected`, `nameSession`, exact `Tab.close`, installed-provider discovery, top-level known-URL shortcut, headless owned tabs, and pre-dispatch validation. `listTabs()`/`getState()` now enumerate all exact ordinary windows of matching Chrome/Edge processes, reconcile every returned CDP page behind stable session-scoped IDs, and keep discovered tabs borrowed. | Facade and discovery are **contract-confirmed**, with invalid-argument **boundary confirmation** against the real helper in epoch 27 and owned headless same-tab **live confirmation** in SET-009. Existing-tab discovery itself is not yet live-accepted; see SET-010. Source/tests: [`src/cua.ts`](../src/cua.ts), [`src/opensky.ts`](../src/opensky.ts), [`test/cua.test.ts`](../test/cua.test.ts), and [`test/typed-browser.test.ts`](../test/typed-browser.test.ts). |
...
| ID | Symptom and cause/layer | Disposition | Validation and evidence |
| --- | --- | --- | --- |
| SDK-042 | Native coordinate scroll could stop at the driver's typed `background_unavailable` pre-input refusal, while click, drag, and typing already retried that exact refusal against the bound window. The public facade ignored an agent-supplied `delivery_mode`; no `bring_to_front` action was involved. | Implemented a single `scroll` replay only for `OpenSkyError.code === "background_unavailable"`, retaining the exact `pid`, `window_id`, coordinates, direction, and amount with `delivery_mode: "foreground"`. Any other error remains unreplayed because delivery may be partial. | **Contract-confirmed** by `test/opensky.test.ts`: the refusal produces one matching foreground retry; an unknown-delivery refusal produces no retry. Real GTK SDK acceptance first proves the target offset moved, then checks a nearby scroller and a same-coordinate foreground sibling remained at zero. The initial `scroll-foreground-01` warm attempt stopped before SDK/app launch because the pinned image lacked `gi`; it is retained as setup evidence. A replacement image is pending before another staged run. |
...
| SET-009 | The initial Playwright smoke drafts either asserted a test backend or used CUA for both action and assertion; neither expressed the requested external setup/action/assertion boundary for browser-local state. OpenSky also lacked a headless exact-tab binding. | OpenSky Driver can now launch a driver-owned headless Chromium process, bind its exact CDP page without a native window, and expose only its loopback debugger endpoint to the trusted host. The Playwright fixture connects to that same tab, loads inline spec-owned HTML, and asserts rendered state; the agent-equivalent `cua` fixture alone performs the action. No local server or backend API is involved. | **Live-confirmed on macOS** for click/text mutation, form-save, and delayed-result workflows; 3/3 cases pass through the public CUA fixture against the real driver and headless Chromium. [`e2e/playwright/basic.spec.ts`](../e2e/playwright/basic.spec.ts), [`e2e/playwright/fixtures.ts`](../e2e/playwright/fixtures.ts). This is same-tab browser-local evidence, not cross-platform coverage. |
| SET-010 | `cua.listTabs()` previously exposed only the facade's private map, so a running user Chrome window was not retrievable without an out-of-band native window ID. Rebinding also minted new provider tab IDs on each scan. | OpenSky requests the bounded existing-profile grant, inventories matching processes and every ordinary window, and reconciles live tabs. OpenSky Driver now keeps one opaque provider tab ID per `(public session, CDP target)` until session teardown. Borrowed tabs remain non-closable. | **Contract-confirmed only**: multi-window enumeration, stable refresh, exact action routing, selected state, disappearance, session cleanup, and borrowed-close refusal pass deterministic SDK/driver tests. A non-disposable local smoke rerun passed two owned-headless cases, then its third fresh runtime failed while repeatedly attaching an unrelated user Chrome process because no exact consent sheet appeared. That is fixture/isolation friction, not existing-tab acceptance; no action was retried. A real disposable two-profile Playwright/provider test remains pending, and no live claim is made for a person's current tabs. |
...
| CLN-005 | “No tabs” was presented as absence of all browser siblings because the facade inventoried only tabs it created or explicitly attached. | `getState()` now publishes `tabInventoryScope:"provider-discovered"` and refreshes running Chrome/Edge inventories. Owned and borrowed bindings remain distinct; tab and SDK cleanup refuse to close borrowed user tabs. Undiscoverable providers/instances are still not proof of global absence. | Provider scope, reconciliation, session cleanup, and borrowed-close refusal are **contract-confirmed** by [`test/cua.test.ts`](../test/cua.test.ts) and [`test/typed-browser.test.ts`](../test/typed-browser.test.ts). Real multi-profile cleanup remains pending under SET-010. |
...
| CLN-009 | A fresh `opensky eval --no-serve` runtime that evaluated pure JavaScript still attempted to end its never-dispatched base session. That unnecessarily resolved/started the configured driver at close and could turn a successful CLI result into exit 1. | Track a sessionless facade separately from a base-session dispatch attempt. Finalization skips only an unused base session; any attempted base call is marked before transport invocation, while managed browser leases, explicit failure receipts, retries, and unresolved ownership remain strict. | Public CLI regression coverage runs `return 1` with a fresh home and nonexistent explicit driver, asserting JSON success and exit 0. Lifecycle/transport ownership tests remain the controls for actual attempted calls. This is non-GUI CLI boundary coverage; it does not establish desktop cleanup behavior. |
...
- Freeze a separate screen-origin candidate campaign on exact driver2e81b62123eda2807ff15edcf6834389dc938c45, build34552663384, binary ecdaf1129eae2cde7f9021aa743ac76e666cb4cdd10075b82b0a278c877e1904. The evaluator/runtime/model/scoring policy remains identical to825b; only the driver artifact pin changes. Four real font-field SDK controls now pass saved60pt with unchanged text and cleanup. Code and Calc regression acceptance remains a pre-dispatch gate. The preceding complete visibility campaign remains16/20OpenSky,14/20native, with one native-only font task; never replace those outcomes. New paired agent smoke starts with that font task after regression acceptance.
...
### LINUX-RELEASE-CAMPAIGN-01 — separate release-binary comparison

The new campaign workflow pins linux-candidate-release-driver from full Linux certification34561195062, exact driver403572f34dfbdc83b17bda7fe4c351060be32785 and independently rehashed release binarye65549f1a2d77d168581a2c2cc84585f90b51748a1cf85c2cf43bfca7a49ba85. All four CI jobs passed; native39/capture7 and installer identities/oracles/cleanup are independently inspected, with shared83 artifact inspection pending. Both arms verify the binary hash and live product/protocol/source identity before budget admission. The scoring profile/native package and task assets remain unchanged. This new source includes attributed9279 generic focus documentation; existing81cd and825b campaigns stay frozen with all first outcomes.

Nine unchanged real SDK cases are staged at runtime source0eaee203da74f674db60b7bbeb64de8017273378 for the verified release binary; their remote acceptance and cleanup remain pending. Changes after0ea are only workflow pins/admission identity checking and this ledger entry. Do not dispatch paid smoke before full artifact inspection and SDK acceptance. Start fresh font/dropdown pairs, then follow the valid-evidence ramp. No paid result or release-agent performance gain is claimed by the CI pass.


### LINUX-CALC-POINTER-CAMPAIGN-01 — preserve the completed baseline when testing the fill correction

The completed release campaign at SDK2d8879f retains its first outcomes: OpenSky17/20 and native15/20, with blank-cell filling the sole native-only success. The fill failure overwrote pre-existing neighboring cells after retained indexed clicks. The isolated Calc pointer correction has supporting real SDK evidence, but these checks do not replace a fresh matched agent comparison.
...
## LINUX-ASSET-CACHE-01 — repeated upstream asset downloads block unrelated tasks

- Symptom/evidence: frozen fcd V2 font08 CI34624268001 failed HTTP429 while fetching unrelated profit workbook assets before admission or agent execution. Full-profile verification and the named-sheet control require the entire pinned asset inventory, so selected-task downloading alone would break existing grading checks.
- Implemented future workflow change: derive an immutable cache key from exact manifest bytes and cache only its40explicit asset files. Use separately pinned `actions/cache/restore` and `save` v4.3.0 at0057852bfaa89a56745cba8c7296529d2fc39830, following this workflow's commit-pin convention. No prefix restore keys; an unexpectedly nonexact restored key refuses. The unchanged fetch-assets.py still checks every asset SHA on hits and downloads missing assets on misses. Save occurs only after the existing install/fetch step succeeds, before unrelated subsequent checks can fail. No entire task directories, evaluator sources outside asset entries, model outputs, or credentials are cached.
- Preserved behavior: fetcher, all-task profile verification, named-sheet control, driver/native pins, reservation admission and agent execution are unchanged. Both native and OpenSky use the same cache; original test.py/test_gold.py are manifest-listed input assets and intentionally included. A new workflow/source is prepared separately; frozen V2 is untouched.
- Validation: YAML parses offline; every run block passes bash syntax; executed manifest-output step against real committed manifest selects exactly40unique files and the expected manifest SHA; original workflow steps/order/fields remain identical after removing the four additive cache steps. Git diff check passes. No cache/CI/GUI/model execution has occurred.
- Tradeoffs: first cold run or eviction still downloads all assets; cache scope follows GitHub branch/default-branch visibility. Successful fetches are saved even if later driver/profile/setup work fails. Cache content is an optimization, never scoring authority: existing corrupt cached bytes fail the fetcher's pin check, rather than being silently trusted or refreshed. An exact key is not overwritten by save; deliberate cache removal/key-version changes require separate operational action if needed.
...
### PASTE-L01 — Linux native multiline paste acceptance

The frozen evaluation build refuses native paste. This candidate routes Linux plain-text paste through the compound driver operation, preserving exact window binding and refreshing after uncertain delivery without replay. The positive consumer workflow uses the existing isolated Writer fixture: select the contextual paragraph, paste two lines, save, and assert the exact text with untouched first/last paragraphs. No mocks, clipboard substitution, injected DriverClient or expected rejection. SDK and E2E typechecks pass. Remote acceptance now passes on SDK9cfbb709de8c87fa34f96f19ca6aadf322c83cc5 and driver445884192f616f4fa3be124a3d4b9148d18adc2c (binary SHA256 e9cfd5cd24d9fc2ae1df2bdc443924cbdb7174dd16c6a8dd0af41a91ebcbd770, built by CI34646617341). PASTE-L01 passed in17.64seconds; independent ODT parsing verified both inserted lines and both unchanged surrounding paragraphs. App exit, temporary removal and disposable-container removal were verified. The fixture disables the ordinary ShowTipOfTheDay preference in its fresh profile, with the exact preferences retained as evidence; this prevents a first-launch popup from intercepting the intended document workflow. The paste method took86ms in this case. Evidence: work/native-paste-sdk-445-independent-check.json and work/native-paste-sdk-445-01. Full-format/race/HTML clipboard cases remain separate; the actor-level five-format restoration check does not replace public-SDK coverage of those cases. This work is isolated from the frozen V3 popup campaign; its driver build and all paid outcomes remain unchanged.
...
### PROGRAM-CONTROL-FLOW-01 — Scoped ordinary control flow for desktop programs

- Symptom/layer: retained Linux Calc split-field evidence rejected a normal indexed row loop before the native REPL could run it. The V5 expression candidate still admitted only straight-line statements. This is an evaluator-policy limitation, not a driver or task result.
- Implemented candidate: admit ordinary `if`, `for`, `for...of`, and `while` statements, lexical local bindings, `++`/`--`, and numeric data indexing while validating every branch and loop body through the existing public desktop API boundary. It passes the original array-row sequence unchanged to the real REPL; there is no code rewriting or task recipe.
- Scope guard: branch joins retain only authority present before and on all paths. Loop analysis computes the descending fixed point `Hnext = H ∩ T(H)` over the test/body/update transfer before admission; it therefore checks every later iteration without a chosen iteration cap. A screenshot path overwritten in iteration one, or shifted through several aliases, cannot authorize a later read or test. Lexical shadows restore the outer binding's capability metadata. Thus conditional or shadowed app bindings and screenshot observations cannot promote global authority, and skipped assignments cannot preserve a screenshot path. Existing literal app selectors, native screenshot URL-only reads, and filesystem/process/network guards remain unchanged.
- Evidence/validation: `e2e/specs/desktop-control-flow-policy.test.ts` exercises `DesktopProgramPolicy` directly for native and OpenSky loops, cross-cell numeric bindings and their failure invalidation, branch-local screenshot reads, joins, shadowing, unauthorized app branches, path overwrites, and repeated-iteration refusals for `for`, `while`, `for...of`, and cascading aliases. The focused policy suites pass 43 tests and SDK plus E2E TypeScript no-emit checks pass locally. Parent-owned `linux-program-repl` fixtures prepare separate real native/OpenSky REPL and saved-document acceptance; those results are pending and are not claimed here.
- Tradeoffs: function declarations, callbacks, arbitrary computed property names, `var` inside control blocks and unconstrained JavaScript remain unsupported. This source is a candidate beyond frozen V5 `b1d68bb`; it does not change frozen outcomes or establish full parity.

### PROGRAM-LOOP-BINDINGS-01 — faithful ordinary `for...of` headers

- Symptom/layer: the frozen V7 native split cell `for (var row of rows)` and the retained OpenSky split cell `for (const [cell, first, last, rank] of rowsA)` were both rejected by evaluator admission before their respective real REPLs ran. The policy advertised `for...of` support, so this was evaluator fidelity friction rather than a driver or fixture limitation.
- Candidate: accept a single ordinary `var`, `let`, or `const` for-of header with either one identifier or a fixed array of identifiers and holes. Iterable values are treated as untrusted ordinary data. A `var` header has JavaScript's function/global lifetime, including an empty iterable and when nested in a branch or lexical loop, so its binding persists to later REPL cells; overwriting it clears app, screenshot, import, path and numeric authority. Lexical headers remain local and restore any shadowed outer capability metadata. Defaults, rest bindings, nested patterns, object patterns and duplicate names stay rejected.
- Validation boundary: focused policy cases cover both retained header shapes, cross-cell `var` use after an empty iterable, branch/enclosing-loop persistence, lexical shadow restoration, stale screenshot/import/app/numeric refusal, and unsupported patterns. These are local admission checks only. Parent-owned real native/OpenSky REPL tests exercise the same ordinary cells and saved document behavior separately; neither policy success nor this document is GUI evidence.
- Limitation: `var` declarations in ordinary blocks and loop bodies remain outside the admitted language, and an earlier read of a header `var` is not modeled before that header has been admitted. The candidate does not model arbitrary destructuring, callbacks, executable defaults or unrestricted JavaScript.

### PROGRAM-HELPERS-01 — Scoped named async desktop helpers

- Symptom/layer: the retained Linux Calc OpenSky run `12-01b269ae-opensky` rejected `async function fillDown(anchor, blanks) { ... }` before it reached the public V8 REPL, even though the same cell's inline-loop recovery later completed and saved all 46 intended values. The frozen native evaluator has the same helper restriction. This is evaluator-policy friction, not proof of a driver or product defect.
- Candidate: admit only top-level, named, non-generator `async function` declarations with simple identifier parameters. The evaluator retains a persistent top-level lexical frame (`name → binding ID`) and stores binding provenance by ID. A direct `await helper(...)` evaluates arguments in the caller, then resolves free names through that declaration frame before introducing parameter bindings. This makes later global declarations and global reassignments visible to the helper, while caller-local shadows cannot replace its captures. Parameters gain app authority only when that call-site argument is the already-scoped fixture app; ordinary data arguments remain ordinary. Calls can use the existing public desktop actions, lexical captures, awaits, and supported loops. Nested/conditional declarations, computed/dynamic calls, callbacks, generators, defaults, destructuring, object mutation, and cyclic helper calls remain rejected.
- Scope guard: helper bodies use the same binding and fixed-point analysis as top-level code. Parameter scope restores after invocation, while mutations to captured top-level bindings are reconciled to the binding-ID authority store before caller locals are restored. This prevents a fabricated object or parameter shadow from acquiring fixture-app authority, and preserves the existing filesystem, import, screenshot-path, selector, and network boundaries. A failed cell clears stored helper definitions because the actual retained body may be uncertain.
- Evidence/validation: the compact packet above records the exact rejected OpenSky source, its pre-execution message, and the successful inline recovery. `desktop-control-flow-policy.test.ts` adds the retained OpenSky helper, a native equivalent, cross-cell invocation, call-time parameter provenance, fabricated-parameter/shadow rejection, declaration hoisting, lexical-capture shadow refusal, forward global capture refusal, global mutation reconciliation, failed-cell invalidation, and definition rebinding invalidation. Three earlier name-based candidates were rejected during parent review for hoisting/capture/caller-shadow authority errors; the rejected patch and all probes remain recorded in work/scoped-helper-parent-review-{1,2,3}.json. The parent rebuilt policy analysis from the frozen baseline around a single transactional LexicalState store, with scope entry/return and branch snapshots carrying binding IDs. All 50 applicable policy and lexical-state tests pass, including the post-call shadow and transactional rollback regressions; the full E2E TypeScript no-emit check passes. Independent read-only review additionally exercised 16 policy assertions. These are local admission checks only. Parent-owned public native/OpenSky REPL and saved-document tests remain the required real-behavior validation.
- Tradeoffs: no unrestricted JavaScript parity is claimed. This candidate is outside frozen source `64b1eff49542f2181a39c63fc2adc6c52bd37c7f`; it changes neither the V8/runtime nor any frozen evaluation outcome. This isolated candidate updates evaluation guidance and rejection text to describe named async helpers; no frozen campaign prompt is changed. Public PROGRAM-L05/L06 use both real REPLs, call the same helper across cells and require exact saved text AAB.Saved.; they are drafted but unrun. No helper runtime acceptance or unrestricted JavaScript parity is claimed.

### PROGRAM-REPL-01 — Validate program execution through both real transports

- Preparation: `linux-program-repl.test.ts` sends the same loop/conditional/template editing workflow through each actual evaluation MCP wrapper. Later REPL cells save the DOCX, and the test inspects exact saved text. No mocks or model calls. The native case uses the existing App observer only to assert the visible save-format dialog; all task input runs through the native REPL.
- Interface: `withOwnedDesktopRepl` generalizes the existing OpenSky-only client, preserving its wrapper and pending-cell handling. Native needs the installed launcher config; no fallback or alternate native backend is allowed. Unexpected reverse permission requests are recorded and refused.
- Execution path: existing Linux workflow `mode=repl`, `repl_case=program`. This exports the config already verified by `linux-environment.ts`, skips model admission and runs the two new cases on a disposable CI desktop. Existing driver/native/scoring pins are unchanged.
- Validation so far: E2E and SDK typechecks, shell syntax, parent replay of both exact prepared programs and the retained native rejected loop through the candidate policy. GUI execution and cleanup receipts are still pending. No frozen campaign adoption or new agent scores.

PROGRAM-REPL-01 first execution: CI34703469492 on190da401 passed2/2. Parent independently verified both saved DOCX files contain exactly `1: First. 3: Third. Saved.`, actual loop/cell receipts, unchanged keyboard maps and successful app/REPL group cleanup. Native final screenshot still shows the format dialog; the previous saved-file assertion could pass before UI completion. Strengthened both cases to wait for that dialog to disappear before reading the saved file. This preserves the first success receipt but leaves final desktop completion pending the strengthened run. No driver/runtime/scorer changes or agent results.


PROGRAM-REPL-01 strengthened execution: CI34703733095 on f05f6c2 failed OpenSky saved-text acceptance: expected `1: First. 3: Third. Saved.`, actual `3: Third. Saved.`. The format dialog closed. Both awaited loop calls appear once and complete successfully in the actual REPL receipts; this does not prove delivery. Native was unrun because the suite bailed after the first failure. The earlier 2/2 run remains retained, with its UI-completion limitation. Driver identity and runtime are unchanged between these attempts; the cause of missing text is unconfirmed.

- Diagnostic follow-up: program-only `--bail=0` allows the independent native case to finish after an OpenSky failure. Optional `OPENSKY_RECORD_INPUT_RESULTS` buffers actual `type_text` and `press_key` driver arguments/results, including structured delivery routes, and writes them when the MCP closes. It adds no input actions, retries, app guidance, replacement results or filesystem waits between input calls. It does not establish that the app consumed an event. Other regressions and paid evaluations leave this diagnostic disabled.
- Validation: SDK and E2E TypeScript no-emit, shell syntax and whitespace checks pass. A fresh real two-backend run remains pending. No timing workaround or driver fix is claimed; frozen V5 and all earlier outcomes remain unchanged.


PROGRAM-REPL-01 diagnostic execution: CI34704358094 on1e7046f failed OpenSky (`Hird. Saved.` saved) and passed native (full text and final dialog closed). Independent saved DOCX parsing and both final screenshots agree; both app/REPL groups exited and keyboard maps match. The three driver typing responses report `route:global_input`, `delivery.mode:background`, `effect:unverifiable`; this confirms the missing text is not explained by a rejected REPL call but does not prove event consumption. The first diagnostic filter missed modifier chords because the SDK sends them as `hotkey`; included that tool for subsequent diagnostics. No shortcut response or actual X11 event/focus sequence was retained in this run. Source diagnosis remains open; no driver fix, benchmark adoption, paid run or score change.


PROGRAM-REPL-01 server-event observation candidate: program-only fixtures now start a passive X11 RECORD observer before opening each real REPL. It records server-delivered keyboard and focus events, their client/window IDs, key/modifier state and timestamps. It sends no input, selects no window events, and changes no app preference or input delay. The original test program and assertions are unchanged. This is diagnostic evidence about server delivery only, not app consumption or a scored fixture service. Both backends use the same observer. The process announces readiness, stops on owned stdin EOF, and retains event/stderr/exit receipts. `python-xlib==0.33` is installed only for the program reproduction; Python API reference is the project's [RECORD example](https://github.com/python-xlib/python-xlib/blob/master/examples/record_demo.py). SDK/E2E no-emit, Python syntax and whitespace checks pass; real observer readiness, event capture and cleanup remain unverified until CI. Frozen driver and V5 remain unchanged.


PROGRAM-REPL-01 recorder first run: CI34704849370 on cfd8350 passed both saved-text/dialog checks and all owned cleanup. It did not reproduce prefix loss. The observer captured nine focus events per backend, but zero core keyboard events; this is inadequate keyboard coverage, not proof of no injected keys. Changed the recorder to subscribe to device KeyPress/Release plus client-delivered FocusIn/Out, following the RECORD device-event interface. Device records establish server-side key order, not application delivery or consumption; focus records are separate evidence. The fixture now rejects a trace with no key press or release during this known keyboard workflow. That explicit diagnostic coverage check prevents another superficially green recorder result. The first attempt and its limitation are retained in work/program-repl-x11-ci-review.json.


PROGRAM-REPL-01 device observation accepted: CI34705099922 on21f361b failed OpenSky saved text (`: First. 3: Third. Saved.`) and passed native's full saved text/final dialog checks. Recorder captured36paired key press/releases for OpenSky and11 for native, with no unmatched releases or pressed keys left at end. The missing `1` has a device event with modifier state0; the stream preserves order and has no recorded focus event during the typing span. This narrows the failure beyond REPL admission and missing XTEST submission, but device records still do not prove client consumption. Native's three type_text calls produced three Ctrl+V chords; OpenSky produced character keys. This motivates evaluating the existing generic clipboard-preserving driver path, not adding a named-app delay. Both app/MCP/recorder process groups exited, final screenshots agree with independent DOCX parsing and keymaps are unchanged. Evidence: work/program-repl-device-ci-review.json plus original CI artifacts. No product fix or frozen benchmark update.


PROGRAM-REPL-01 hold-timing candidate: passive traces show OpenSky shortcut key press/release at the same server timestamp while native holds each key for about33ms. Exact frozen driver `send_key_xtest` queues key-down, sleeps its existing10ms, then queues release and flushes. Driver candidate57d1b03e046021dbdaca459602c5734170295693 adds a server round trip after key-down so that existing hold interval can take effect. It changes no text payload, clipboard behavior, app setting or delay constant. Native paste remains a separate public operation: its clipboard authorization, payload limits and newline semantics make automatic substitution inappropriate here. Existing Linux typing workflow now builds this candidate in release mode, matching the frozen binary optimization profile, and runs its existing focused Rust checks in release mode. First dispatch must use `build_only=true`; subsequent real public REPL acceptance must pin the actual returned binary hash. No new agent score or root-cause claim; all prior failures remain.
...
PROGRAM-LOOP-BINDINGS-01 real-interface correction: CI34721003447 on d0d4b7a admitted both previously rejected loop forms. Both PROGRAM-L03/L04 then failed with `Cannot read properties of undefined` because the test incorrectly expected a header `var` to survive into another async REPL cell. OpenSky persists program-level declarations only (`src/async-repl.ts`); native produced the same runtime error. Existing PROGRAM-L01/L02 both passed and all owned cleanup passed. Retain this failed attempt. The revised tests check header-var reuse after an empty loop within the same cell, then flat destructuring of the program-level array in a later cell and exact saved text. No runtime, native cell behavior or task score changes. Fresh GUI validation pending.

PROGRAM-LOOP-BINDINGS-01 second real attempt: CI34721274659 on569a93d passed native PROGRAM-L04 and both existing controls. OpenSky PROGRAM-L03 completed the editing cells but refused the final Enter with foreground_unavailable: its bound app still named the document window while the format dialog was active. The new test observed the dialog through the external fixture observer, leaving the acting REPL handle stale. The bundled skill specifies observing after window transitions; src/cua.ts updates the bound handle on observations. The corrected test uses the acting OpenSky REPL's getAXState, matching existing PROGRAM-L01. This is a test correction, not a driver focus-policy change. Preserve the failure; fresh validation pending.
