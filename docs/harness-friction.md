...
- Fix: in progress in isolated driver branch codex/linux-index-identity. Bind the observed target to its AT-SPI object and owning frame, scope to runtime/PID/window/snapshot, reject a disappeared target before input, and preserve exact-window guards. No candidate acceptance or promotion yet.
