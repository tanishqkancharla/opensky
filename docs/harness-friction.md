...
### LINUX-NATIVE-PASTE-CERTIFICATION-01 — exact candidate Linux regression gate

Pin the existing full Linux certification workflow to OpenSky Driver ae1577dd1faebd55385b16fe1a9c5453dceec4ee. This differs from tested445884192 only in acceptance documentation; native paste actor/input matches9c7272351. Ordinary Linux CI passed, public SDK multiline saved paste and sibling-focus guard passed, and independent real clipboard clients verified restoration plus pre-input and after-input external ownership changes. The latter returns honest uncertainty and can paste the newer owner's HTML; no successful-delivery claim is substituted for that result.

This wrapper changes only the three exact driver-source pins. It runs unchanged canonical shared, native, capture and installer lanes on GitHub-hosted Ubuntu; no filtered cells, paid model, or modification to frozen V3 evaluations. All canonical artifacts and the exact shared-lane release binary are retained. Runtime results remain pending at preparation. Remaining Linux formats/transfers and other native platforms are explicit wider scope; this gate alone does not establish native-paste or agent-task parity.
