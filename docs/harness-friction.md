...
The public native paste implementation unconditionally refuses non-browser targets. Added a positive consumer workflow using the existing isolated Writer fixture: select the contextual paragraph, paste two lines, save, and assert the exact text with untouched first/last paragraphs. No mocks, clipboard substitution, injected DriverClient or expected rejection. E2E typecheck and diff checks pass; no desktop run or passing acceptance is claimed yet. Full-format/race/HTML clipboard cases remain separate. Candidate is isolated from the frozen2d/403paidcampaign. Measured fill-sequence failure now takes replay priority before staging this baseline.
