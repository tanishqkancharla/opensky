...
### SUBSCRIPT35 — Distinguish direct selection from the exact four-arrow workflow

Release arm35 saved subscript on six O characters and two x characters, leaving all eight requested 2s normal while preserving document text. All recorded selectText/format calls completed, and saved formatting proves this was not simply an unsaved or no-op change. The selection/arrow cause remains unresolved; the agent's intended digit selection is not ground truth. The prior native arrow control used five different arrows and does not establish this exact four-arrow behavior.

Two new isolated no-model Writer cases retain the same disposable three-paragraph ODT fixture and saved-file oracle. SUBSCRIPT35-L01 directly selects 2 with H/O context, observes, replaces it with MARK, and saves; only that digit may change. SUBSCRIPT35-D01 selects H2O, observes, presses Right/Left/Left/Shift+Right exactly as arm35, then replaces and saves. Its expected O replacement is a diagnostic prediction from arm35's actual formatting, not a prescribed native behavior or new acceptance claim. The fixture retains final pixels, actual ODT, saved selected-character diagnostic, and owned app/temp cleanup even on assertion failure. Runtime src and all driver/evaluation pins remain frozen 2d; there is no paid dispatch or repair sequence.
...
### ASSET-SETUP-429-01 — unrelated task download blocks a paid worker before admission

Current frozen fcd/ed9 font native07 CI34623812556 passed; independent saved PPTX inspection confirms both requested60/28pt target sizes with all text and unrelated formatting preserved. OpenSky08 CI34624268001 failed in Install desktop and scorer, downloading an unrelated Calc IncomeStatement2.xlsx. The frozen fetcher visits every task and exhausts1s/2s retry delays after HTTP429. Reservation admission and agent execution were both skipped; this is unscored infrastructure failure, not a failed user task.

Retained fresh CI identity, failed log and pre-admission proof under08-impress-font-opensky/linux-office. A narrow local adaptation of the existing pre-admission reconciler released only its$5reservation using proven no-agent zero accounting (no model usage receipt is claimed). Other ledger entries and campaign summary remain unchanged; no score/result was fabricated. Current settled estimate145.798191 and approved cap200 were revalidated. Recovery requires a new one-use reservation/arm at identical frozen source, retaining original08, and an explicit additive selection declaration before pairing. Never rerun a spent reservation envelope or replace a valid failure. Durable task-scoped downloads with hash-verified cache and Retry-After-aware backoff remain proposed for a future source revision.

### SCORECARD-VERSIONS-01 — fixed coverage and independent partial completion

The original scorecard showed completed paired tasks as a changing percentage denominator and swapped campaign scope between five and20. The user requested stable version history. The local scorecard now registers V1 (release2d/403) and V2 (currentfcd/ed9), always renders the same20task IDs, and retains explicit Passed/Failed/Not run/Running/Awaiting result/Not scored states. Main rates use each backend's valid completions, including unpaired outcomes; matched metrics remain separate. Selected raw evidence, SDK/driver/native-package hashes, Linux platform, cleanup and source membership are checked. Earlier outcomes are not carried forward.

Actual-data verification: V1 OpenSky17/20 versus native15/20; V2 OpenSky2/3 versus native3/4, with08unscored and three matched pairs. All state counts sum20 per side, unselected outcomes are excluded, wrong-source evidence is refused, and timestamps include all versions. Browser verification confirmed version switching,20rows, counts, defaultV2 and readable layout. This is scorecard validation, not new desktop acceptance. Widening coverage at identical software remains the same evaluation version; changed tested software requires a new version and retention of the old selection.


ASSET-SETUP-429-01 recovery update: original five-selection bytes and393original evidence files are retained by additive declaration setup-recovery-08-impress-font-opensky.json (SHA f26a744d5af27bfa0f036a96fe77e2a8aff9d728ba9de548026d108d9c06afe0). The replacement is a separately reserved first-agent attempt at unchanged fcd, CI34628942535, paired with retained native07; runtime result is pending. One shared read-only resolver supplies the declared substitution to both adopter and tail. Four real pairs are verified; the pending fifth correctly blocks adoption. Nine negative validation checks reject altered declaration/mapping/types/path/protected proof/settlement/scored-original evidence. This is infrastructure-helper validation, not a desktop acceptance result.

Current Code09/10 paired task completed successfully: OpenSky CI34628157314 took56964ms/10calls/$0.5858574; native CI34628509171 took22840ms/5calls/$0.1942442. Each saved file exactly equals original with all10 text→test replacements, every other byte preserved, and the frozen gold hash. Cleanup/usage verified; comparison fingerprints have no differences. V2 now has four matched tasks,3/4passes per side. Original dropdown failures remain unchanged. Cumulative settled146.5782926, cap200; slide recovery holds one$5reservation.
