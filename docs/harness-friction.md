...
### CALC-SELECTION-BUILD-01 — isolate an exact candidate build

The existing fill replay workflow still pinned driver2ff75 while its runtime and tests exercised release403. The remote machine has no warm Rust cache and limited disk. This isolated workflow pins the experimental30189c6 driver, builds release, and keeps the existing focused Rust checks. The paid frozen campaign stays unchanged. Compilation and the unchanged public SDK saved-workbook acceptance are pending; successful Selection acknowledgments alone are insufficient.
...
Build34592310556 compiles release and passes78 existing focused tests plus6 popup units. All3 realX11 controls failed during fixture activation before capture assertions. Inspection found the intended desktop-dependency edit had not matched the existing libwayland-dev line: Openbox was never explicitly installed, and its startup log was discarded. Correct the dependency list, verify executable availability/WM process/EWMH readiness, and retain startup/test logs. Keep driver040 and live fixture unchanged for this retry; a map-focus race is only a hypothesis. Separate unit/live/release steps and run release optimization after live controls to surface setup failures earlier. Original logs remain retained.
