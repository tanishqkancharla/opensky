...
### FILL-L01 / FILL-L02 — Preserve existing cells during consecutive Calc edits

Release campaign arm `12-calc-fill-opensky` filled the requested 46 blanks but also overwrote C12, C16, C22 and C25. The native arm passed. Retained initial and post-batch AX traces associate the submitted indices with the preceding blank cells; none of the four populated cells was explicitly targeted. Corruption was already visible before correction or saving. This localizes the failure to the initial indexed click/type/Enter sequence, but does not distinguish pointer targeting from edit/commit timing.

The new public-SDK cases replay C10 then C11 on the original workbook, with a whole-workbook saved-value/formula comparison that also checks every untargeted cell and sheet order. FILL-L01 uses indices from one observation with no observation between edits. FILL-L02 uses the visible name box as a control. Existing cell tests checked their targets without this preservation assertion. Both tests use real owned Calc instances and isolated documents. No runtime fix, acceptance claim or campaign regrade is made; remote replay is pending. Source runtime remains identical to frozen `2d8879f`, driver `403572f`. Screenshot coordinate controls retain the existing 1280×881/883 environment constraint.

Remote `calc-fill-replay-before-01` reproduced the defect on exact driver 403572f: FILL-L01 saved C10 and incorrectly overwrote C12, leaving C11 blank; FILL-L02 changed only C10/C11. Both app groups exited and fetched Docker inventory was empty. The saved-workbook oracle independently confirms these differences. FILL-D01 is a separate phase-observation diagnostic; it adds screenshots after the first Enter and the retained C11 click without refreshing AX indices, to distinguish click targeting from subsequent typing/commit. The timing fixture now retains screenshots returned by explicitly requested public screenshot actions. Original FILL-L01/L02 action sequences and evidence remain preserved.

FILL-D01 (`calc-fill-phase-before-01`) reproduces the same saved failure. Its screenshots show C11 selected immediately after the first Enter, then C12 selected immediately after the indexed C11 click and before typing. This isolates the failure to click targeting. FILL-D02 adds bounded read-only AT-SPI identity and raw component geometry probes in fixture diagnostics; it does not change the public input sequence, driver binary or scored campaign. Its purpose is to distinguish wrong logical-cell identity from wrong geometry, not to assert internal RPC traffic as acceptance. The original no-probe regression remains authoritative for the saved outcome.

FILL-D02's first remote attempt (`calc-fill-identity-before-01`) failed before test input because `python-dbus` is absent from the frozen desktop image. Cleanup completed; this is diagnostic infrastructure failure, not another product outcome. The helper now uses the existing GI Gio/GLib D-Bus API with explicit signatures, keeping the image and production runtime unchanged. The original failed diagnostic remains retained; the corrected helper requires a new frozen stage.

The corrected FILL-D02 on a separate diagnostic image (`calc-fill-identity-before-03`, same 403572f driver) reproduced the saved failure and kept C11's bus/path and Position(10,2) unchanged. C11 Screen top changed from363 to387 after editing while Window top stayed363; the frame reported Screen(0,0), Window(0,-17), and X11 client origin was(0,17). The existing per-component rebase therefore adds17 again after editing. Initial raw Screen coordinates are also offset from the screenshot, so globally removing rebasing would be unsafe. The next read-only probe compares parent-table hit tests at raw and rebased centers, using verified PID/XID ownership and no fixed application offset. GI image adds four diagnostic packages with no upgrades; original full-desktop and campaign images remain unchanged. The optional GetRowColumnSpan observation returned a different tuple shape; Position supplied the row/column evidence and no span-based acceptance is claimed.

The spatial-lookup diagnostic (`calc-fill-hit-test-before-01`) rules out using table hit tests as an unconditional physical oracle: before editing, the raw C10 center(258,353) is reported as C10 even though the screenshot places the physical cell lower; the current rebased center(258,370) is reported as C11 but the actual click selects C10. After editing, raw C11 center(258,398) agrees with the physical cell, and rebased(258,415) selects C12. Table Screen top shifts153→177 while Window top stays153. The table lookup shares the initial stale coordinate origin. This proposal is therefore not implemented. Original no-probe FILL-L01 remains the regression; no new product fix or acceptance claim is made. All owned apps and containers exited after these runs.


### FILL-L03 / FILL-L04 — Preserve ordinary cell-click focus and range behavior

The first table-selection candidate marked the requested cell without moving Calc's typing cursor, and a second selection extended the marked range. The replacement candidate uses cell focus only when no range is marked. Existing FILL-L01/L02 cover consecutive edits but do not explicitly start with name-box focus or an existing marked range. CALC-L05 selects its range after the indexed click, so it does not cover this boundary either.
...
### FILL-L06 — Check indexed targeting after moving the Calc window

The proposed geometry correction must distinguish a maximized window at the screen origin from a displaced window. Existing consecutive-cell controls start maximized. FILL-L06 repeats the original two indexed edits in an owned window restored, resized to 960×700 and moved to (200,100) through ordinary window-manager actions before the first SDK observation. Setup reuses the EWMH restore approach from the existing sibling-window fixture and xdotool placement from the Code-origin fixture, verifies PID/XID ownership before and after, independently checks displaced client coordinates and dimensions, and retains window-layout.json. No fixed decoration inset or driver-returned bounds establish setup correctness.

The test retains both indices from one initial AX observation and requires exactly C10/C11 to change, preserving every other value/formula and sheet name. Optional placement leaves all original fixture paths and prior test bodies unchanged. E2E typechecking and diff checks pass; the reused restore script also passes Python syntax compilation. The new moved-window setup and workflow have not yet run on a desktop, so this is pending acceptance coverage, not proof of the geometry candidate. Existing owned app/temp cleanup remains responsible for the moved window.


### FILL-D03 — Diagnose the moved-window first-click failure

The geometry-only candidate d3005 passed FILL-L01–L05 in calc-geometry-six-after-01 but failed unchanged FILL-L06: it saved C9/C11 instead of C10/C11. The independently recorded moved client was (201,118), 960×700. This remains a failed positive contract; it does not justify changing the expected cells or promoting the candidate.
...
### FILL-D04 — Isolate GTK motion-driven coordinate refresh

Both frozen 403 and d300 fail the moved-window D03 first click before typing, selecting C9 instead of C10. Exact upstream LibreOffice 24.2.7.2 source shows that [GTK configure stores top-level geometry](https://github.com/LibreOffice/core/blob/libreoffice-24.2.7.2/vcl/unx/gtk3/gtkframe.cxx#L4001-L4032), while [motion updates it from root minus event-box coordinates](https://github.com/LibreOffice/core/blob/libreoffice-24.2.7.2/vcl/unx/gtk3/gtkframe.cxx#L3742-L3763). The [menu inserts a GTK grid row above that event box](https://github.com/LibreOffice/core/blob/libreoffice-24.2.7.2/vcl/unx/gtk3/gtksalmenu.cxx#L995-L1005). This explains a possible shared Screen-origin update without a cell identity change; it does not establish a universal 24-pixel correction or prove Ubuntu's runtime takes that path.

The distinct FILL-D04 diagnostic repeats D03 but, after its first retained AX/probe, verifies owned PID/XID and active window, retains the X11 child tree, and moves the pointer once to the observed C10 raw Screen center without a button or key. It immediately retains another bounded read-only probe before returning the original AX result. This is an explicit diagnostic intervention; old tests and the whole-workbook C10/C11 oracle remain unchanged. It requires the existing GI-enabled diagnostic image. xdotool --sync confirms pointer movement, not application event processing, so an unchanged immediate probe cannot exclude a later refresh. No arbitrary inset, observation retry, focus activation, or alternate click is introduced.

Local typechecking/diff validation are required before staging; no desktop run or new product acceptance is claimed. The setup remains within the disposable owned application and preserves the existing cleanup lifecycle.

### CALC-REFRESH-01 — fresh visible cell rejected after observation

The corrected driver's first blank-fill agent run CI34583825229 passed the saved outcome: exactly46originally blank cells changed, with all existing values and sheet order preserved. However calls5and7 rejected freshly advertised B3 index1063 as an absent AT-SPI object, including after call6's full fresh AX observation. The agent recovered using screenshot coordinates. This is a task success, not causal proof that indexed-cell targeting is fully corrected. Initial unnamed-panel1476 input accidentally edited A1/A2; the agent undid both changes before the B3 refusals.

REFRESH-L01 exercises a fresh B3 index on an untouched workbook; REFRESH-L02 first performs and undoes the same A1 fill-down through public keyboard actions. Each then saves and asserts onlyB3changed. The second setup deliberately uses Ctrl+Home rather than a guessed unnamed panel index, isolating the workbook edit/undo condition. These are real SDK/workbook tests with the existing fixture; no product changes or fake driver receipts. Source src remains byte-identical to frozenfcd/2d. Remote execution and mechanism attribution are pending.


### CALC-REFRESH-HOSTED-01 — reproduce refreshed-index refusal on the original hosted desktop

The corrected fill agent saved the requested workbook, but two B3 indexed clicks refused after about three seconds, including one after a fresh full observation. The driver may have omitted the target during a timed-out child traversal; actual object recreation is still possible. Existing REFRESH-L01/L02 test clean observation and the observed edit/fill-down/Undo posture through the public SDK, with whole-workbook preservation assertions. Remote Docker results alone cannot establish hosted-runner behavior.

The isolated linux-typing.yml bridge now downloads the exact certified release artifact from34580005558 (ed9fd15e39a1d2acd07c4f3078440c3ab2ad44f2; binary SHA2568b50bfbdbd9c98aaa5d0ff9d4f2196b18115b96fe6c2d68847ddb16c21ce6692) without rebuilding. It uses the existing hosted desktop installation helper and a private Xvfb/D-Bus session, runs only both unchanged REFRESH cases once with bail0/retry0, and enables CUA_ATSPI_DEBUG. The focused runner retains live daemon verification, source/binary hashes, host/package versions, debug logs, screenshots, saved workbooks, test results and owned-process/temporary cleanup. It uses the registered workflow path to avoid a new-workflow dispatch-registration dependency; no paid campaign or runtime is changed.

Local workflow/shell parsing and exact test/source diff checks are preparation evidence only. No hosted execution, new runtime acceptance or cause attribution is claimed until the resulting artifact is inspected. The ordinary install helper still fetches VS Code and the existing asset fetcher verifies the full manifest; those setup dependencies are preserved to match the hosted setup and may delay this diagnostic. Debug logging can itself affect timing, so a clean result does not rule out the original timeout.

Hosted CI34585926656 passed unchangedREFRESH-L01/L02 with independently checked exactbinary/source/runtime, onlyB3savedchanges and fullcleanup. Clicks took2983ms/2967ms; logs had nochildtimeout/error/truncation. This doesnot reproduce the original3003/3002msfailure. REFRESH-L03 retains the original formula-toolbar firstpanelclick and rejectednativequery before the edit/fill-down/Undo/freshB3sequence, whichL02simplified toCtrl+Home. The next hosted run selects onlythisnewcase and expectsonefixturecleanup; originaltworesults remain unchanged. No productfixorpaidcampaignchange.

CI34586872442 failed before any click: the fixture's readiness observation had already primed the SDK diff cache, so the initial default observation correctly returned `No accessibility changes.` The test incorrectly expected a full tree. REFRESH-L03 now explicitly requests `disableDiffing: true` for its initial observation. This uses the public API without changing product behavior or user settings. The original failed receipt is retained with exact source9b88f17, certified binary8b50, and successful owned app/driver/temp cleanup. The stale B3 refusal remains unreproduced; the corrected test still needs remote execution.
