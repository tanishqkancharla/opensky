...
### DROPDOWN-ALLOWED-CONSUMER-01 — distinguish typed allowed values from saved list construction

Retained OpenSky dropdown05 on frozenfcd/ed9 created three displayed Entries via typeText and OK. Two Fail values committed, but Pass plus Enter produced an Invalid value alert; choosing Pass from the dropdown later worked. The later added trailing newline explains the final fourth blank choice independently. Existing DROPDOWN-L02/L03 check saved rule construction but do not consume an allowed value. Normal Calc AutoInput could complete Pass to the existing Pass/Fail/Held header; the retained modal-only observation does not prove the text actually submitted.

New DROPDOWN-C01/C02 reuse the pinned original workbook, existing List fixture and public SDK. Both enter Fail twice. C01 then types Pass and preserves actual screenshot/AX before Enter; C02 chooses the first declared unsorted list option through ordinary dropdown keys. Both require only D2/D3/D4 to change to Fail/Fail/Pass and all28three-choice validation rules to remain exact. AutoInput and other user settings are untouched. This is a deliberately discriminating consumer contract, not a claim that literal typing must defeat the application's normal completion semantics. A C01 failure requires inspection of the precommit selection/completion before attributing fault.

The isolated test checkout starts at fcd01bb0a12a9004c83407bb26549e7ae7401025. Product/runtime/evaluator/workflow inputs and existing fixtures/tests remain unchanged. Only new fixture/spec plus this record are drafted. Local typecheck/diff review is preparation only; no GUI/model execution, hosted dispatch or new acceptance has occurred. On failure the existing fixture retains final screenshot/AX, saved original file and owned-app cleanup; the new observation files retain the unsaved editor state that the saved workbook cannot reveal. Setup remains the established coordinate-based Entries selection, whereas the agent used indexed Entries; this first control isolates allowed-value consumption and does not prove equivalence of every setup path.
...
### DROPDOWN-POPUP-OBSERVATION-01 — bracket app capture with a read-only desktop observer

The retained C02 consumer succeeds with exact saved values and rules, but its app-scoped screenshot shows a black popup. A different native run's full-desktop screenshot shows options; different states/scopes cannot establish a capture defect. DROPDOWN-D01 is a separate diagnostic, not an SDK parity acceptance assertion. It reuses the unchanged original List/Entries setup, opens the popup through public Alt+Down, and captures full app AX/PNG, ImageMagick root PNG, then full app AX/PNG without intervening input. It records before/after active XID/PID, verifies the owned process start identity and XID owner, and retains root window trees, window geometry, observer exit codes and timestamps. Active identity changes or failed capture commands fail the diagnostic rather than inventing an outcome. A popup without a PID property is retained explicitly as a nonfatal observer result.

The root observer invokes the same `import -window root png:-` primitive used by the existing X11 display-capture fallback, but remains fixture-only. No removed ScreenshotTool or unrelated capture_audio operation is invoked. Sequential images are not atomic; independent inspection of both bracketing app images and window inventories is required before assigning cause. Diagnostic completion alone is not a claim that options are visible or that app capture is defective. Ordinary Escape follows the bracket, and inherited owned-app cleanup and failure artifacts remain in place.

This isolated branch starts at cae3ac7 with runtime identical to a6b54dc/fcd. No production, evaluator, existing test, fixture or campaign changes. Local TypeScript/diff/staging checks are preparation only; no desktop/model run or new acceptance is claimed.


### DROPDOWN-POPUP-OWNERSHIP-01 — retain exact popup metadata before designing composition

D01 independently retained two byte-identical app PNGs (black popup) around a root capture showing Pass/Fail/Held, with unchanged active PID/XID and owned geometry. Root-tree popup 0x802d64 changed height71 to61 during that bracket; its relationship to the exact workbook was not yet proven. Preserve that first source and artifact.

Separate D02 adds read-only metadata to a copy of the D01 observer: enumerate at most64 direct root children, retain each child's xwininfo map/override-redirect/geometry and raw result, then xprop _NET_WM_PID, WM_TRANSIENT_FOR, WM_CLIENT_LEADER, _NET_WM_WINDOW_TYPE and WM_CLASS for viewable or LibreOffice-labelled candidates. Exact owned-window properties are also captured. Title/class only selects diagnostic candidates; it is never ownership authorization. Missing properties, vanished windows and failed optional metadata commands remain explicit receipts. Before/after candidate changes are reported without sleeping until a preferred outcome or claiming atomic captures. All capture-bracket operations remain read-only, and inherited app cleanup is preserved.

The original D01 fixture/spec are unchanged. D02 is a separate diagnostic and does not assert screenshot parity, authorize composition, or change a product path. The retained D01 tree verifies the bounded direct-child parser finds21 root children including the popup, excluding its nested child. Local TypeScript/diff and immutable staging checks are preparation only. D02 is not executed; its stage uses the separately inspected imagemagick-only diagnostic image2e3a8a80, unchanged runtimea6 and certified drivered9/hash8b50.

### DROPDOWN-POPUP-PIXELS-01 — compare the actual human-visible dropdown

D02 completed on certified driver ed9/hash8b50 in the isolated diagnostic image2e3a8a80. The viewable override-redirect COMBO has the requested frame's exact WM_TRANSIENT_FOR and PID, while its height settles71→61. Both app screenshots hide popup pixels that the intervening desktop screenshot shows. Evidence: linux-office-setup/dropdown-popup-metadata-ed9-before-01/independent-popup-metadata-check.json. This is a screenshot defect, separate from the frozen agent task's saved-outcome failure.

POPUP-L01 opens the same normal three-choice dropdown through the public SDK and compares both public app screenshots with the independent visible desktop. The observer derives the region from real same-owner metadata, checks foreground identity and unchanged app geometry, retains the entire capture bracket and compares only the common popup rectangle if its bottom edge settles. No OCR, expected-coordinate fixture, alternate input route, retry, or product setting is used. A meaningful nonuniform desktop region and preserved app canvas are required. A maximum channel difference8 and mismatched-pixel fraction1% are frozen before candidate execution; raw differences and images are retained. Sequential capture cannot establish atomicity, and pixel agreement does not itself score agent completion. The first old-driver run and later unchanged candidate run must both be retained. Typecheck passes; execution is pending.
