...
export async function withNativeSky(artifacts: string, use: (sky: NativeSky) => Promise<void>): Promise<void> {
  ...
  const cell = async (code: string): Promise<Result> => {
    const result = await request("tools/call", { name: "js", arguments: { code, title: "Native Impress click and keyboard control" } }) as Result;
  ...
  const sky: NativeSky = {
    async click(args) { await cell(`await sky.click(${JSON.stringify(args)});`); },
    ...
    async get_screenshot() {
      ...
      const path = join(artifacts, `native-${String(++images).padStart(3, "0")}.${image.mimeType === "image/png" ? "png" : "jpg"}`);
