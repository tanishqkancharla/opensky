...
export const test = base.extend<Fixture & { owned: Fixture }>({
  owned: async ({ task }, use) => {
    ...
    const artifacts = resolve(process.env.OPENSKY_LINUX_OFFICE_ARTIFACT!, "selection-arrows", task.name.split(":")[0]);
    await mkdir(artifacts, { recursive: true });
    const temporary = await mkdtemp(join(tmpdir(), "opensky-selection-arrows-"));
      ...
      await withOwnedLinuxApp({ executable: "/usr/bin/libreoffice", documentTitle: "selection-arrows.odt", artifacts,
        ...
        try {
          ...
          await writeFile(join(artifacts, "saved-selection-diagnostic.json"), JSON.stringify({
            ...
            note: "Saved replacement identifies the edited character. Compare native Text and ordinary Find cases before attributing a failure to the driver.",
          }, null, 2));
        }
      });
    } finally {
