...
def sibling():
    ...
    # Cover the target at the same screenshot coordinate. A route that sends
    # the initial background scroll to the foreground sibling is observable.
    window.set_default_size(820, 600)
    window.move(40, 40)
    box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
    box.set_border_width(16)
    window.add(box)
    sibling_status = Gtk.Label(label="Sibling offset: 0")
    box.pack_start(sibling_status, False, False, 0)
    box.pack_start(scroller("Sibling", sibling_status), True, True, 0)
    window.connect("destroy", Gtk.main_quit)
    window.show_all()
    Gtk.main()
