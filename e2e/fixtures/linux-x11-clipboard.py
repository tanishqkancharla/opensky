...
"""
import argparse, ctypes as C, json, os, signal, subprocess, sys, time
...
REPLACEMENT_FORMATS = [
    ("UTF8_STRING", "UTF8_STRING", 8, bytes("Replacement clipboard Δ", "utf8")),
    ("text/html", "text/html", 8, bytes("<i>Replacement clipboard Δ</i>", "utf8")),
    ("application/x-opensky-test-binary", "application/x-opensky-test-binary", 8, bytes([222,173,190,239,68])),
    ("application/x-opensky-test-u16", "INTEGER", 16, bytes([34,34,51,51,85,85])),
    ("application/x-opensky-test-u32", "CARDINAL", 32, bytes([239,190,173,222,4,3,2,1])),
]
...
def wait_for_replacement(d, clipboard, state, process):
    deadline = time.monotonic() + 5
    while time.monotonic() < deadline:
        if process.poll() is not None:
            raise RuntimeError(f"replacement clipboard client exited before ownership: {process.returncode}")
        try:
            with open(state) as f: replacement = json.load(f)
            if int(X.XGetSelectionOwner(d, clipboard)) == replacement["owner"]:
                return replacement
        except (FileNotFoundError, json.JSONDecodeError):
            pass
        time.sleep(.002)
    raise RuntimeError("replacement clipboard client did not acquire CLIPBOARD")
def owner(state, report, profile, race=None):
    d = display(); w = window(d); clipboard = atom(d, "CLIPBOARD"); targets = atom(d, "TARGETS")
    formats = REPLACEMENT_FORMATS if profile == "replacement" else FORMATS
    ...
    with open(state, "w") as f: json.dump({"owner": w, "pid": os.getpid(), "profile": profile, "formats": [{"target":t,"type":ty,"width":width,"bytesHex":raw.hex()} for t,ty,width,raw in formats]}, f)
    requests = []
    try:
        while not stopped[0]:
            ...
            if r.selection != clipboard or r.owner != w: prop = 0
                ...
                if value is None: prop = 0
                    ...
                    if race and child is None and r.target == atom(d, "application/x-opensky-test-u32"):
                        replacement_state, replacement_report, evidence = race
                        child = subprocess.Popen([sys.executable, os.path.abspath(__file__), "--owner", replacement_state, replacement_report, "--profile", "replacement"])
                        replacement = wait_for_replacement(d, clipboard, replacement_state, child)
                        race_observation = {
                            "initialOwner": w,
                            "triggerTarget": atom_name(d, r.target),
                            "replacementOwner": replacement["owner"],
                            "replacementPid": replacement["pid"],
                            "ownerBeforeReply": int(X.XGetSelectionOwner(d, clipboard)),
                        }
                        with open(evidence, "w") as f: json.dump(race_observation, f)
        ...
        race_child = {"stopped": True, "reportExists": False}
        if child is not None:
            if child.poll() is None: child.terminate()
            try: child.wait(timeout=5)
            except subprocess.TimeoutExpired:
                child.kill(); child.wait(timeout=2)
            race_child = {"pid": child.pid, "exitCode": child.returncode, "stopped": child.returncode is not None,
                          "reportExists": bool(race and os.path.exists(race[1]))}
        X.XDestroyWindow(d, w); X.XCloseDisplay(d)
        with open(report, "w") as f: json.dump({"requests": requests, "raceObservation": race_observation, "raceChild": race_child, "released": True}, f)
def read_all():
...
parser.add_argument("--profile", choices=("original","replacement"), default="original")
parser.add_argument("--race", nargs=3, metavar=("STATE","REPORT","EVIDENCE"))
args = parser.parse_args()
if args.owner: owner(*args.owner, args.profile, args.race)
