...
export const test = base.extend<Fixture & { owned: Fixture }>({
  owned: async ({ task }, use) => {
    ...
    const artifacts = resolve(process.env.OPENSKY_LINUX_OFFICE_ARTIFACT!, "subscript35-selection", task.name.split(":")[0]);
      ...
      await withOwnedLinuxApp({ executable: "/usr/bin/libreoffice", documentTitle: "subscript35-selection.odt", artifacts,
        ...
        try {
          ...
          await writeFile(join(artifacts, "saved-selection-diagnostic.json"), JSON.stringify({
            ...
            note: "Saved replacement identifies the edited character. This read-only saved outcome distinguishes the selected character; native Linux has no matching selectText API, and a Find setup is a distinct condition.",
          }, null, 2));
        }
      });
    } finally {
